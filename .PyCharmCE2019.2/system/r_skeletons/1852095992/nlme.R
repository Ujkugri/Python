...
lmeControl <- function (maxIter = 50, msMaxIter = 50, tolerance = 1e-06, niterEM = 25, 
    ...
    natural = TRUE, sigma = NULL, allow.n.lt.q = FALSE, ...) 
{
    if (is.null(sigma)) 
        sigma <- 0
    ...
    list(maxIter = maxIter, msMaxIter = msMaxIter, tolerance = tolerance, 
        ...
        natural = natural, sigma = sigma, allow.n.lt.q = allow.n.lt.q, 
        ...)
}
...
groupedData <- function (formula, data = NULL, order.groups = TRUE, FUN = function(x) max(x, 
    na.rm = TRUE), outer = NULL, inner = NULL, labels = NULL, 
    units = NULL) 
{
...
nmGroupedData <- function (formula, data = NULL, order.groups = TRUE, FUN = function(x) max(x, 
    na.rm = TRUE), outer = NULL, inner = NULL, labels = NULL, 
    units = NULL) 
{
    if (!local({
        N <- -1L
        while (identical(sys.call(N)[[1]], quote(eval))) N <- N - 
            1L
        identical(.sf <- sys.function(N), groupedData) || identical(environment(.sf), 
            .ns) || identical(environment(sys.function(N - 1L)), 
            .ns) || identical(environment(sys.function(N - 2L)), 
            .ns)
    })) 
...
nfGroupedData <- function (formula, data = NULL, order.groups = TRUE, FUN = function(x) max(x, 
    na.rm = TRUE), outer = NULL, inner = NULL, labels = NULL, 
    units = NULL) 
{
    if (!local({
    ...
    if (order.groups && !inherits(groups, "ordered")) {
        levs <- if (is.null(outer)) {
            ...
            as.vector(unlist(lapply(split(data.frame(response = response, 
                groups = groups), combined), function(obj, func) {
                names(sort(tapply(obj$response, obj$groups, func)))
            }, func = FUN)))
        }
    ...
    attr(data, "FUN") <- FUN
    attr(data, "order.groups") <- order.groups
    cl <- if ((length(all.vars(getCovariateFormula(formula))) == 
        0) || (data.class(primary) != "numeric")) {
        "nffGroupedData"
    }
    else {
        "nfnGroupedData"
    }
    class(data) <- unique(c(cl, "nfGroupedData", "groupedData", 
        class(data)))
    data
}
    ...
    naPattern, control = list(), verbose = FALSE) 
{
    ...
    if (missing(params)) {
        ...
        params <- list(formula(paste(paste(pNams, collapse = "+"), 
            "1", sep = "~")))
    }
    ...
    params <- unlist(lapply(params, function(pp) {
        if (is.name(pp[[2]])) {
            ...
            eval(parse(text = paste("list(", paste(paste(all.vars(pp[[2]]), 
                deparse(pp[[3]]), sep = "~"), collapse = ","), 
                ")")))
        }
    }), recursive = FALSE)
    ...
    for (nm in pnames) {
        if (deparse(params[[nm]][[3]]) != "1") {
            ...
            plist[[nm]] <- model.matrix(form1s, model.frame(form1s, 
                dataModShrunk))
    ...
    repeat {
        ...
        work <- .C(fit_gnls, thetaNLS = as.double(spar), as.integer(unlist(Dims)), 
            ...
            nlModel, NAOK = TRUE)[c("thetaNLS", "settings", "additional")]
    ...
    if ((sigma <- controlvals$sigma) == 0) {
        ...
        lsig <- log(sigma)
        loglik <- -(NReal * (log(2 * pi)/2 + lsig) + sum((c.L$Xy[, 
            pLen + 1])^2)/(2 * sigma^2)) + c.L$logLik
    }
    ...
    repeat {
        ...
        if (controlvals$opt == "nlminb") {
            ...
            if (optRes$convergence && controlvals$msWarnNoConv) 
                warning(paste(sprintf("Iteration %d, LME step: nlminb() did not converge (code = %d).", 
                  numIter, optRes$convergence), if (convIter >= 
                  control$iter.max) 
            ...
            if (aNlm$code > 2 && controlvals$msWarnNoConv) 
                warning(paste(sprintf("Iteration %d, LME step: nlm() did not converge (code = %d).", 
                  numIter, aNlm$code), if (aNlm$code == 4) 
                  "Do increase 'msMaxIter'!"))
        }
        ...
        for (i in 1:Q) {
            ...
            w <- w + as.vector((ZX[, startRan + 1:ncols[i], drop = FALSE] * 
                t(sran[[i]])[gr.i, , drop = FALSE]) %*% rep(1, 
                ncols[i]))
            startRan <- startRan + ncols[i]
        }
        if (decomp) {
...
corSpatial <- function (value = numeric(0), form = ~1, nugget = FALSE, type = c("spherical", 
    ...
    "maximum", "manhattan"), fixed = FALSE) 
{
    spClass <- c(spherical = "corSpher", exponential = "corExp", 
        gaussian = "corGaus", linear = "corLin", rational = "corRatio")[match.arg(type)]
    structure(value, formula = form, nugget = nugget, metric = match.arg(metric), 
        fixed = fixed, class = c(spClass, "corSpatial", "corStruct"))
}
...
.skeleton_package_version = "3.1-139"
