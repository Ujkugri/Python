    ...
    demean = TRUE, intercept = demean, series = NULL, ...) 
{
    ...
    res <- list(order = m, ar = ar, var.pred = var.pred, x.mean = xm, 
        x.intercept = xint, aic = xaic, n.used = n.used, n.obs = n.used, 
        order.max = order.max, partialacf = NULL, resid = E, 
        method = "Unconstrained LS", series = series, frequency = xfreq, 
        call = match.call(), asy.se.coef = list(x.mean = sem, 
            ar = drop(ses)))
    class(res) <- "ar"
    res
}
...
reformulate <- function (termlabels, response = NULL, intercept = TRUE, env = parent.frame()) 
{
    ...
    fexpr <- if (is.null(response)) 
        call("~", terms)
    else call("~", if (is.character(response)) tryCatch(str2code(response), 
        error = function(e) {
            sc <- sys.calls()
            sc1 <- lapply(sc, `[[`, 1L)
