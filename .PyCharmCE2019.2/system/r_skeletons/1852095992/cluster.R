...
daisy <- function (x, metric = c("euclidean", "manhattan", "gower"), stand = FALSE, 
    type = list(), weights = rep.int(1, p), warnBin = warnType, 
...
mona <- function (x, trace.lev = 0) 
{
    ...
    x <- (if (iM) 
        apply(x, 2, iF)
    else vapply(x, iF, integer(n))) - 1L
    hasNA <- anyNA(x)
    if (hasNA) 
        x[is.na(x)] <- 2L
    res <- .Fortran(cl_mona, as.integer(n), as.integer(p), x = x, 
        error = as.integer(trace.lev), nban = integer(n), ner = integer(n), 
        integer(n), lava = integer(n), integer(p))
    if (res$error != 0) {
    ...
    structure(class = "mona", list(data = res$x, hasNA = hasNA, 
        order = res$ner, variable = res$lava[-1], step = res$nban[-1], 
...
clusGap <- function (x, FUNcluster, K.max, B = 100, d.power = 1, spaceH0 = c("scaledPCA", 
...
diana <- function (x, diss = inherits(x, "dist"), metric = "euclidean", 
    stand = FALSE, stop.at.k = FALSE, keep.diss = n < 100, keep.data = !diss, 
    trace.lev = 0) 
{
    if ((diss <- as.logical(diss))) {
        if (anyNA(x)) 
    ...
    stopifnot(length(trace.lev <- as.integer(trace.lev)) == 1)
    stopifnot(is.logical(stop.at.k) || (is.numeric(stop.at.k) && 
        1 <= stop.at.k && stop.at.k <= n))
    C.keep.diss <- keep.diss && !diss
    res <- .C(twins, n, jp, as.double(x2), dv, dis = double(if (C.keep.diss) length(dv) else 1), 
        jdyss = if (C.keep.diss) diss + 10L else as.integer(diss), 
        if (mdata) rep(valmisdat, jp) else double(1), if (mdata) jtmd else integer(jp), 
        as.integer(ndyst), 2L, as.integer(stop.at.k), integer(n), 
        ner = integer(n), ban = double(n), dc = double(1), double(1), 
        merge = matrix(0L, n - 1, 2), trace = trace.lev)
    if (!diss) {
        if (res$jdyss == -1) 
...
clara <- function (x, k, metric = c("euclidean", "manhattan", "jaccard"), 
    stand = FALSE, samples = 5, sampsize = min(n, 40 + 2 * k), 
    ...
    pamLike = FALSE, correct.d = TRUE) 
{
    ...
    res <- .C(cl_clara, n, jp, k, clu = as.double(x), samples, 
        ...
        as.logical(rngR[1]), as.logical(pamLike[1]), as.integer(dFlag), 
        integer(sampsize), integer(sampsize), sample = integer(sampsize), 
        integer(k), imed = integer(k), double(k), double(k), 
        double(k), avdis = double(k), maxdis = double(k), ratdis = double(k), 
        size = integer(k), obj = double(1), avsil = double(k), 
        ttsil = double(1), silinf = matrix(0, sampsize, 4), jstop = integer(1), 
...
pam <- function (x, k, diss = inherits(x, "dist"), metric = c("euclidean", 
    "manhattan"), medoids = NULL, stand = FALSE, cluster.only = FALSE, 
    do.swap = TRUE, keep.diss = !diss && !cluster.only && n < 
        100, keep.data = !diss && !cluster.only, pamonce = FALSE, 
    trace.lev = 0) 
{
    ...
    if (do.swap) 
        nisol[1] <- 1L
    res <- .Call(cl_Pam, k, n, !diss, if (diss) dv else x2, !cluster.only, 
        medoids, do.swap, trace.lev, keep.diss, pamonce, if (mdata) rep(valmisdat, 
            jp) else double(1), if (mdata) jtmd else integer(jp), 
        ndyst)
...
.skeleton_package_version = "2.0.8"
