...
compile <- function (e, env = .GlobalEnv, options = NULL, srcref = NULL) 
{
    ...
    if (mayCallBrowser(e, cntxt)) 
        ...
        genCode(e, cntxt)
    else genCode(e, cntxt, loc = list(expr = e, srcref = srcref))
}
    ...
    options = NULL, version = NULL) 
{
    ...
    if (nforms > 0) {
        ...
        for (i in 1:nforms) {
            ...
            if (!mayCallBrowser(e, cntxt)) 
                cforms[[i]] <- genCode(e, cntxt, loc = list(expr = e, 
                  srcref = sref))
        }
    ...
    for (n in nm) {
        ...
        switch(n, optimize = {
            ...
            if (isTRUE(op) || isFALSE(op)) {
                old <- c(old, list(suppressNoSuperAssignVar = compilerOptions$suppressNoSuperAssignVar))
