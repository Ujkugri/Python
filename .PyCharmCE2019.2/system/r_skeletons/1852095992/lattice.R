...
lrect <- function (xleft, ybottom, xright, ytop, x = (xleft + xright)/2, 
    ...
    font, fontface, ..., identifier = NULL, name.type = "panel") 
{
    ...
    alpha = add.line$alpha, type, ..., reference = FALSE, identifier = "abline") 
{
    ...
    if (!is.null(a)) {
        ...
        sign.dist.from.line <- function(x, y) {
            as.integer(sign(zapsmall(y - coeff[1] - coeff[2] * 
                x)))
        }
...
panel.densityplot <- function (x, darg = list(n = 512), plot.points = "jitter", ref = FALSE, 
...
USMortality <- lattice::USMortality		## Mortality Rates in US by Cause and Gender

USRegionalMortality <- lattice::USRegionalMortality		## Mortality Rates in US by Cause and Gender
...
.skeleton_package_version = "0.20-38"
