...
read.spss <- function (file, use.value.labels = TRUE, to.data.frame = FALSE, 
    ...
    reencode = NA, use.missings = to.data.frame, sub = ".", add.undeclared.levels = c("sort", 
        "append", "no"), duplicated.value.labels = c("append", 
        "condense"), duplicated.value.labels.infix = "_duplicated_", 
    ...) 
{
    add.undeclared.levels <- match.arg(add.undeclared.levels)
    duplicated.value.labels <- match.arg(duplicated.value.labels)
    trim <- function(strings, trim = TRUE) if (trim && is.character(strings)) 
        sub(" +$", "", strings)
    else strings
    ...
    if (reencode) 
        names(vl) <- iconv(names(vl), cp, "", sub = sub)
    ...
    for (v in has.vl) {
        ...
        if (reencode && nlabels) {
            ...
            names(vl[[v]]) <- names(vlv) <- iconv(nm2, cp, "", 
                sub = sub)
        }
        ...
        if (use.value.labels && (add.undeclared.levels != "no" || 
            all(uniquevalues %in% c(newlevels, ""))) && (!is.finite(max.value.labels) || 
            nvalues <= max.value.labels)) {
            newlabels <- rev(trim(names(vlv), trim.factor.names))
            if (add.undeclared.levels != "no" && !all(uniquevalues %in% 
                c(newlevels, ""))) {
                addlabels <- addlevels <- sort(uniquevalues[!(uniquevalues %in% 
                  c(newlevels, ""))])
                ...
                warning("Undeclared level(s) ", paste(addlevels, 
                  collapse = ", "), " added in variable: ", nm)
            }
            ...
            if (any(dupnewlabels)) {
                ...
                if (duplicated.value.labels == "append") {
                  newlabels[dupnewlabels] <- paste(newlabels[dupnewlabels], 
                    newlevels[dupnewlabels], sep = duplicated.value.labels.infix)
                }
            ...
            rval[[nm]] <- factor(newrval, levels = newlevels, 
                labels = newlabels)
        }
...
.skeleton_package_version = "0.8-71"
