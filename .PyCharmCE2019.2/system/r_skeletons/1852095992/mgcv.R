...
dDeta <- function (y, mu, wt, theta, fam, deriv = 0) 
{
    ...
    d <- list(Deta = 0, Dth = 0, Dth2 = 0, Deta2 = 0, EDeta2 = 0, 
        Detath = 0, Deta3 = 0, Deta2th = 0, Detath2 = 0, Deta4 = 0, 
        Deta3th = 0, Deta2th2 = 0)
    if (fam$link == "identity") {
        d$Deta <- r$Dmu
        d$Deta2 <- r$Dmu2
        d$EDeta2 <- r$EDmu2
        d$Deta.Deta2 <- r$Dmu/r$Dmu2
        ...
        if (deriv > 0) {
            ...
            g3g <- fam$g3g(mu)
            d$Deta3 <- r$Dmu3 * ig13 - 3 * r$Dmu2 * g2g * ig12 + 
                r$Dmu * (3 * g2g^2 - g3g) * ig1
            if (!is.null(r$EDmu3)) 
                d$EDeta3 <- r$EDmu3 * ig13 - 3 * r$EDmu2 * g2g * 
                  ig12
            d$Deta2th <- r$Dmu2th * ig12 - r$Dmuth * g2g * ig1
            if (!is.null(r$EDmu2th)) 
                d$EDeta2th <- r$EDmu2th * ig12
        }
        if (deriv > 1) {
            g4g <- fam$g4g(mu)
            d$Deta4 <- ig12^2 * r$Dmu4 - 6 * r$Dmu3 * ig13 * 
                g2g + r$Dmu2 * (15 * g2g^2 - 4 * g3g) * ig12 - 
                r$Dmu * (15 * g2g^3 - 10 * g2g * g3g + g4g) * 
                  ig1
            d$Dth2 <- r$Dth2
            d$Detath2 <- r$Dmuth2 * ig1
            d$Deta2th2 <- ig12 * r$Dmu2th2 - r$Dmuth2 * g2g * 
                ig1
            d$Deta3th <- ig13 * r$Dmu3th - 3 * r$Dmu2th * g2g * 
                ig12 + r$Dmuth * (3 * g2g^2 - g3g) * ig1
        }
    }
    good <- is.finite(d$Deta) & is.finite(d$Deta2)
    if (deriv > 0) {
        if (length(theta) > 1) 
            good <- good & is.finite(rowSums(d$Dth)) & is.finite(rowSums(d$Detath)) & 
                is.finite(rowSums(d$Deta2th)) & is.finite(d$Deta3)
        else good <- good & is.finite(d$Dth) & is.finite(d$Detath) & 
            is.finite(d$Deta2th) & is.finite(d$Deta3)
        if (deriv > 1) {
            if (length(theta) == 1) 
                good <- good & is.finite(d$Dth2) & is.finite(d$Detath2) & 
                  is.finite(d$Deta2th2) & is.finite(d$Deta3th) & 
                  is.finite(d$Deta4)
            else good <- good & is.finite(rowSums(d$Dth2)) & 
                is.finite(rowSums(d$Detath2)) & is.finite(rowSums(d$Deta2th2)) & 
    ...
    np = TRUE, xt = NULL, id = NULL, sp = NULL, pc = NULL) 
{
    ...
    ret <- list(margin = margin, term = term, by = by.var, fx = fx, 
        label = label, dim = dim, np = np, id = id, sp = sp, 
        inter = FALSE)
    if (!is.null(pc)) {
        if (length(pc) < d) 
    ...
    100) 
{
    ...
    Dd <- function(y, mu, theta, wt, level = 0) {
        ...
        if (level > 0) {
            ...
            r$EDmu2th <- r$Dmu2th <- 2 * r$Dmu2 + 2 * wt * theta^3 * 
                (mu * psi2.muth + onemu * psi2.onemuth)
        }
        if (level > 1) {
    ...
    postproc <- function(family, y, prior.weights, fitted, linear.predictors, 
        offset, intercept) {
        theta <- family$getTheta(trans = TRUE)
        ...
        posr$null.deviance <- 2 * lf$f + sum(family$dev.resids(y, 
            wtdmu, prior.weights))
        posr$family <- paste("Beta regression(", round(theta, 
            3), ")", sep = "")
        posr
    }
    ...
    residuals <- function(object, type = c("deviance", "working", 
        ...
        if (type == "working") {
            ...
            res <- 2 * lf$term + object$family$dev.resids(y, 
                mu, wts)
            res[res < 0] <- 0
            s <- sign(y - mu)
            res <- sqrt(res) * s
    ...
    environment(dev.resids) <- environment(aic) <- environment(getTheta) <- environment(rd) <- environment(qf) <- environment(variance) <- environment(putTheta) <- environment(saturated.ll) <- environment(preinitialize) <- env
...
FFdes <- function (size = 5, ccd = FALSE) 
{
    ...
    if (ccd && size > 1) {
        design <- rbind(design, diag(size) * sqrt(size), -diag(size) * 
            sqrt(size))
    }
    design
}
    ...
    diagonalize = FALSE) 
{
    ...
    lambda <- initial.spg(G$X, G$y, G$w, family, G$S, G$rank, 
...
scat <- function (theta = NULL, link = "identity", min.df = 3) 
{
    ...
    if (!is.null(theta) && sum(theta == 0) == 0) {
        ...
        if (sum(theta < 0)) {
            iniTheta <- c(log(abs(theta[1]) - min.df), log(abs(theta[2])))
        }
        else {
    ...
    Dd <- function(y, mu, theta, wt, level = 0) {
        ...
        if (level > 0) {
            ...
            fymf1 <- fym * f1
            ymsig2a <- ym/sig2a
            oo$EDmu2th <- oo$Dmu2th <- oo$Dmuth <- oo$Dth <- matrix(0, 
                n, 2)
            ...
            oo$Dmu2th[, 2] <- 4 * wt * (-nu1nusig2a + ff1 * 5 - 
                4 * ff1 * f1ym)
            oo$EDmu3 <- rep(0, n)
            oo$EDmu2th <- cbind(4/(sig^2 * (nu + 3)^2) * exp(theta[1]), 
                -2 * oo$EDmu2)
        }
        if (level > 1) {
            nu1nu <- nu1/nu
    ...
    postproc <- function(family, y, prior.weights, fitted, linear.predictors, 
        offset, intercept) {
        posr <- list()
        posr$null.deviance <- find.null.dev(family, y, eta = linear.predictors, 
            offset, prior.weights)
        th <- round(family$getTheta(TRUE), 3)
        if (th[1] > 999) 
            th[1] <- Inf
        posr$family <- paste("Scaled t(", paste(th, collapse = ","), 
            ")", sep = "")
        posr
    }
    rd <- function(mu, wt, scale) {
    ...
    environment(dev.resids) <- environment(aic) <- environment(getTheta) <- environment(Dd) <- environment(ls) <- environment(rd) <- environment(variance) <- environment(putTheta) <- env
...
mvn <- function (d = 2) 
{
    ...
    preinitialize <- function(G) {
        ...
        attr(G$X, "XX") <- XX
        if (!is.null(G$Sl)) 
            attr(G$Sl, "E") <- cbind(attr(G$Sl, "E"), matrix(0, 
                nbeta, ntheta))
    ...
    ll <- function(y, X, coef, wt, family, offset = NULL, deriv = 0, 
        d1b = NULL, d2b = NULL, Hp = NULL, rank = 0, fh = NULL, 
        D = NULL) {
        if (!is.null(offset)) {
            for (i in 1:length(offset)) if (sum(offset[[i]] != 
                0)) 
                stop("mvn does not yet handle offsets")
        }
        lpi <- attr(X, "lpi")
    ...
    ...) 
{
    ...
    if (inherits(family, "extended.family")) 
        warning("gamm is not designed to use extended families")
    ...
    if (!is.null(object$pred.formula)) 
        environment(object$pred.formula) <- .GlobalEnv
    ret$gam <- object
    ...
    modCon = 0) 
{
    ...
    if (!is.null(sm$g.index) && is.null(sm$C)) {
        sm$C <- matrix(colMeans(sm$X), 1, ncol(sm$X))
        if (length(sm$S)) {
            upen <- rowMeans(abs(sm$S[[1]])) == 0
            if (length(sm$S) > 1) 
                for (i in 2:length(sm$S)) upen <- upen & rowMeans(abs(sm$S[[i]])) == 
                  0
            if (sum(upen) > 0) 
                drop <- min(which(upen))
            else {
                  ...
                  for (i in 1:length(sml)) {
                    if (length(sm$S) > 0) 
                      for (l in 1:length(sm$S)) {
                        ...
                        if (nz > 0) 
                          ZSZ[, indi[1:nz]] <- t(qr.qty(qrc, 
                            t(ZSZ[, indi, drop = FALSE]))[(nc + 
                            1):nx, ])
...
{
    if (length(link) != 3) 
        stop("gevlss requires 3 links specified as character strings")
    okLinks <- list(c("log", "identity"), "identity", c("identity", 
        "logit"))
    stats <- list()
    for (i in 1:3) {
        if (link[[i]] %in% okLinks[[i]]) 
            stats[[i]] <- make.link(link[[i]])
        else stop(link[[i]], " link not available for mu parameter of gaulss")
    ...
    if (link[[3]] == "logit") {
        stats[[3]]$linkfun <- function(mu) binomial()$linkfun((mu + 
            1)/1.5)
        stats[[3]]$mu.eta <- function(eta) binomial()$mu.eta(eta) * 
            1.5
        stats[[3]]$linkinv <- function(eta) 1.5 * binomial()$linkinv(eta) - 
            1
        stats[[3]]$d2link <- function(mu) {
            mu <- (mu + 1)/1.5
            (1/(1 - mu)^2 - 1/mu^2)/1.5^2
        }
        stats[[3]]$d3link <- function(mu) {
            mu <- (mu + 1)/1.5
            (2/(1 - mu)^3 + 2/mu^3)/1.5^3
        }
        stats[[3]]$d4link <- function(mu) {
            mu <- (mu + 1)/1.5
            (6/(1 - mu)^4 - 6/mu^4)/1.5^4
        }
    }
    residuals <- function(object, type = c("deviance", "pearson", 
        ...
        type <- match.arg(type)
        if (type == "deviance") {
            rsd <- (xi + 1)/xi * log(1 + (y - mu) * exp(-rho) * 
                xi) + (1 + (y - mu) * exp(-rho) * xi)^(-1/xi) + 
                (1 + xi) * log(1 + xi) - (1 + xi)
            rsd <- sqrt(pmax(0, rsd)) * sign(y - fv)
        }
        else if (type == "pearson") {
            sd <- exp(rho)/xi * sqrt(pmax(0, gamma(1 - 2 * xi) - 
                gamma(1 - xi)^2))
            rsd <- (y - fv)/sd
        }
        else {
    ...
    ll <- function(y, X, coef, wt, family, offset = NULL, deriv = 0, 
        ...
        etar <- X[, jj[[2]], drop = FALSE] %*% coef[jj[[2]]]
        rho <- family$linfo[[2]]$linkinv(etar)
        etax <- X[, jj[[3]], drop = FALSE] %*% coef[jj[[3]]]
        xi <- family$linfo[[3]]$linkinv(etax)
        eps <- 1e-07
        xi[xi >= 0 & xi < eps] <- eps
        xi[xi < 0 & xi > -eps] <- -eps
        n <- length(y)
        ...
        aa0 <- (xi * ymu)/exp1^rho
        ind <- which(aa0 <= -1)
        if (FALSE && length(ind) > 0) {
            xii <- xi[ind]
            erho <- exp1^rho[ind]
            eps1 <- 1 - .Machine$double.eps^0.25
        ...
        if (deriv > 0) {
            bb1 <- 1/exp1^rho
            bb2 <- bb1 * xi * ymu + 1
            l1[, 1] <- (bb1 * (xi + 1))/bb2 - bb1 * bb2^((-1/xi) - 
                1)
            cc2 <- ymu
            cc0 <- bb1 * xi * cc2
            log.cc3 <- log1p(cc0)
            cc3 <- cc0 + 1
            l1[, 2] <- (-bb1 * cc2 * cc3^((-1/xi) - 1)) + (bb1 * 
                (xi + 1) * cc2)/cc3 - 1
            dd3 <- xi + 1
            dd6 <- 1/cc3
            dd7 <- log.cc3
            dd8 <- 1/xi^2
            l1[, 3] <- (-(dd8 * dd7 - bb1 * aa2 * cc2 * dd6)/cc3^aa2) + 
                dd8 * dd3 * dd7 - aa2 * dd7 - bb1 * aa2 * dd3 * 
                cc2 * dd6
            l2 <- matrix(0, n, 6)
            ee1 <- 1/exp1^(2 * rho)
            ee3 <- -1/xi
            l2[, 1] <- ee1 * (ee3 - 1) * xi * aa1^(ee3 - 2) + 
                (ee1 * xi * (xi + 1))/aa1^2
            ff7 <- ee3 - 1
            l2[, 2] <- bb1 * cc3^ff7 + ee1 * ff7 * xi * cc2 * 
                cc3^(ee3 - 2) - (bb1 * dd3)/cc3 + (ee1 * xi * 
                dd3 * cc2)/cc3^2
            gg7 <- -aa2
            l2[, 3] <- (-bb1 * cc3^(gg7 - 1) * (log.cc3/xi^2 - 
                bb1 * aa2 * cc2 * dd6)) + ee1 * cc2 * cc3^(gg7 - 
                2) + bb1 * dd6 - (ee1 * (xi + 1) * cc2)/cc3^2
            hh4 <- cc2^2
            l2[, 4] <- bb1 * cc2 * cc3^ff7 + ee1 * ff7 * xi * 
                hh4 * cc3^(ee3 - 2) - (bb1 * dd3 * cc2)/cc3 + 
                (ee1 * xi * dd3 * hh4)/cc3^2
            l2[, 5] <- (-bb1 * cc2 * cc3^(gg7 - 1) * (log.cc3/xi^2 - 
                bb1 * aa2 * cc2 * dd6)) + ee1 * hh4 * cc3^(gg7 - 
                2) + bb1 * cc2 * dd6 - (ee1 * (xi + 1) * hh4)/cc3^2
            jj08 <- 1/cc3^2
            jj12 <- 1/xi^3
            jj13 <- 1/cc3^aa2
            l2[, 6] <- (-jj13 * (dd8 * dd7 - bb1 * aa2 * cc2 * 
        ...
        if (deriv > 1) {
            ...
            kk1 <- 1/exp1^(3 * rho)
            kk2 <- xi^2
            l3[, 1] <- (2 * kk1 * kk2 * (xi + 1))/aa1^3 - kk1 * 
                (ee3 - 2) * (ee3 - 1) * kk2 * aa1^(ee3 - 3)
            ll5 <- (xi * cc2)/exp1^rho + 1
            ll8 <- ee3 - 2
            l3[, 2] <- (-2 * ee1 * ff7 * xi * ll5^ll8) - kk1 * 
            ...
            oo10 <- gg7 - 1
            oo13 <- log.cc3/xi^2
            l3[, 5] <- bb1 * cc3^oo10 * (bb1 * oo10 * cc2 * dd6 + 
        ...
        if (deriv > 3) {
            ...
            uu1 <- 1/exp1^(4 * rho)
            uu2 <- xi^3
            l4[, 1] <- uu1 * (ee3 - 3) * (ee3 - 2) * (ee3 - 1) * 
                uu2 * aa1^(ee3 - 4) + (6 * uu1 * uu2 * (xi + 
                1))/aa1^4
            vv09 <- ee3 - 3
            l4[, 2] <- 3 * kk1 * ll8 * ff7 * kk2 * ll5^vv09 + 
            ...
            ww12 <- cc3^(gg7 - 4)
            ww15 <- cc3^ww11
            l4[, 3] <- (-kk1 * mm11 * oo10 * kk2 * ww15 * (log.cc3/kk2 - 
                ...
                dd3 * cc2)/cc3^4
            l4[, 4] <- 4 * ee1 * ff7 * xi * ll5^ll8 + 5 * kk1 * 
            ...
            l4[, 5] <- (-2 * ee1 * oo10 * xi * mm12 * (bb1 * 
                ...
                rho) * cc3^4)
            l4[, 6] <- ee1 * oo10 * xi * mm12 * tt18^2 - 2 * 
            ...
            l4[, 7] <- bb1 * cc3^ff7 + 7 * ee1 * ff7 * xi * cc2 * 
            ...
            l4[, 8] <- (-bb1 * cc3^oo10 * (bb1 * oo10 * cc2 * 
            ...
            l4[, 9] <- bb1 * ad20 * (bb1 * oo10 * cc2 * dd6 + 
            ...
            l4[, 10] <- (-bb1 * pp08 * ae16^3) - 3 * bb1 * pp08 * 
            ...
            af05 <- cc2^4
            l4[, 11] <- bb1 * cc2 * cc3^ff7 + 7 * ee1 * ff7 * 
            ...
            ah27 <- tt18^2
            l4[, 13] <- bb1 * cc2 * ad20 * ah27 + ee1 * oo10 * 
                ...
                af05)/cc3^4
            l4[, 14] <- (-bb1 * cc2 * pp08 * ae16^3) - 3 * bb1 * 
    ...
    initialize <- expression({
        ...
        if (is.null(start)) {
            ...
            startji <- qr.coef(qr(x1), c(rep(family$linfo[[3]]$linkfun(0.001), 
                nrow(x1))))
            startji[!is.finite(startji)] <- 0
            start[jj[[3]]] <- startji
        }
    })
    structure(list(family = "gevlss", ll = ll, link = paste(link), 
        nlp = 3, tri = trind.generator(3), initialize = initialize, 
        ...
        for (i in 1:m) {
            if (sm[[i]]$dim == d && sm[[i]]$side.constrain) {
                ...
                for (j in 1:d) {
                  ...
                  k <- (1:length(b))[b == i]
                  if (k > 1) 
                    for (l in 1:(k - 1)) if (!b[l] %in% X1comp) {
                      X1comp <- c(X1comp, b[l])
                      if (with.pen) {
...
tw <- function (theta = NULL, link = "log", a = 1.01, b = 1.99) 
{
    ...
    ls <- function(y, w, theta, scale) {
        a <- get(".a")
        b <- get(".b")
        LS <- w * ldTweedie(y, y, rho = log(scale), theta = theta, 
            a = a, b = b)
        LS <- colSums(LS)
        lsth1 <- c(LS[4], LS[2])
    ...
    postproc <- function(family, y, prior.weights, fitted, linear.predictors, 
        ...
        posr$null.deviance <- find.null.dev(family, y, eta = linear.predictors, 
            offset, prior.weights)
        posr$family <- paste("Tweedie(p=", round(family$getTheta(TRUE), 
            3), ")", sep = "")
        posr
    }
    rd <- function(mu, wt, scale) {
...
plot.gam <- function (x, residuals = FALSE, rug = NULL, se = TRUE, pages = 0, 
...
ocat <- function (theta = NULL, link = "identity", R = NULL) 
{
    ...
    postproc <- function(family, y, prior.weights, fitted, linear.predictors, 
        ...
        posr$null.deviance <- find.null.dev(family, y, eta = linear.predictors, 
            offset, prior.weights)
        posr$family <- paste("Ordered Categorical(", paste(round(family$getTheta(TRUE), 
            2), collapse = ","), ")", sep = "")
        posr
    }
    validmu <- function(mu) all(is.finite(mu))
    ...
    preinitialize <- function(y, family) {
        ...
        if (!is.numeric(y)) 
            stop("Response should be integer class labels")
...
anova.gam <- function (object, ..., dispersion = NULL, test = NULL, freq = FALSE) 
{
    ...
    if (length(dotargs) > 0) {
        if (!is.null(test) && !test %in% c("Chisq", "LRT", "F")) 
            stop("un-supported test")
        if (is.list(object$formula)) 
            object$formula <- object$formula[[1]]
        n <- if (is.matrix(object$y)) 
            nrow(object$y)
        else length(object$y)
        dfc <- if (is.null(object$edf2)) 
            0
        else sum(object$edf2) - sum(object$edf)
        ...
        for (i in 1:length(dotargs)) {
            if (is.list(dotargs[[i]]$formula)) 
                dotargs[[i]]$formula <- dotargs[[i]]$formula[[1]]
            dfc <- if (is.null(dotargs[[i]]$edf2)) 
                0
            else sum(dotargs[[i]]$edf2) - sum(dotargs[[i]]$edf)
            dotargs[[i]]$df.residual <- n - sum(dotargs[[i]]$edf1) - 
                dfc
...
gamlss.etamu <- function (l1, l2, l3 = NULL, l4 = NULL, ig1, g2, g3 = NULL, g4 = NULL, 
    i2, i3 = NULL, i4 = NULL, deriv = 0) 
{
    K <- ncol(l1)
    d1 <- l1
    for (i in 1:K) {
    ...
    if (deriv > 0) 
        for (i in 1:K) for (j in i:K) for (l in j:K) {
            ...
            mo <- max(ord)
            if (mo == 3) {
                d3[, k] <- (l3[, k] - 3 * l2[, i2[i, i]] * g2[, 
                  i] * ig1[, i] + l1[, i] * (3 * g2[, i]^2 * 
                  ig1[, i]^2 - g3[, i] * ig1[, i])) * ig1[, i]^3
            }
            else if (mo == 1) {
                d3[, k] <- l3[, k] * ig1[, i] * ig1[, j] * ig1[, 
                  l]
            }
            else {
                k1 <- ii[ord == 1]
                k2 <- ii[ord == 2]
                d3[, k] <- (l3[, k] - l2[, i2[k2, k1]] * g2[, 
                  k2] * ig1[, k2]) * ig1[, k1] * ig1[, k2]^2
            }
        }
    k <- 0
    d4 <- l4
    if (deriv > 2) 
        for (i in 1:K) for (j in i:K) for (l in j:K) for (m in l:K) {
            ...
            mo <- max(ord)
            if (mo == 4) {
                d4[, k] <- (l4[, k] - 6 * l3[, i3[i, i, i]] * 
                  g2[, i] * ig1[, i] + l2[, i2[i, i]] * (15 * 
                  g2[, i]^2 * ig1[, i]^2 - 4 * g3[, i] * ig1[, 
                  i]) - l1[, i] * (15 * g2[, i]^3 * ig1[, i]^3 - 
                  10 * g2[, i] * g3[, i] * ig1[, i]^2 + g4[, 
                  i] * ig1[, i])) * ig1[, i]^4
            }
            else if (mo == 1) {
                d4[, k] <- l4[, k] * ig1[, i] * ig1[, j] * ig1[, 
                  l] * ig1[, m]
            }
            else if (mo == 3) {
                k1 <- ii[ord == 1]
                k3 <- ii[ord == 3]
                d4[, k] <- (l4[, k] - 3 * l3[, i3[k3, k3, k1]] * 
                  g2[, k3] * ig1[, k3] + l2[, i2[k3, k1]] * (3 * 
                  g2[, k3]^2 * ig1[, k3]^2 - g3[, k3] * ig1[, 
                  k3])) * ig1[, k1] * ig1[, k3]^3
            }
            else {
                if (sum(ord == 2) == 2) {
                  k2a <- (ii[ord == 2])[1]
                  k2b <- (ii[ord == 2])[2]
                  d4[, k] <- (l4[, k] - l3[, i3[k2a, k2b, k2b]] * 
                    g2[, k2a] * ig1[, k2a] - l3[, i3[k2a, k2a, 
                    k2b]] * g2[, k2b] * ig1[, k2b] + l2[, i2[k2a, 
                    k2b]] * g2[, k2a] * g2[, k2b] * ig1[, k2a] * 
                    ig1[, k2b]) * ig1[, k2a]^2 * ig1[, k2b]^2
                }
                else {
                  k2 <- ii[ord == 2]
                  k1a <- (ii[ord == 1])[1]
                  k1b <- (ii[ord == 1])[2]
                  d4[, k] <- (l4[, k] - l3[, i3[k2, k1a, k1b]] * 
                    g2[, k2] * ig1[, k2]) * ig1[, k1a] * ig1[, 
                    k1b] * ig1[, k2]^2
                }
            }
        }
...
k.check <- function (b, subsample = 5000, n.rep = 400) 
{
    ...
    for (k in 1:m) {
        ...
        if (!is.null(attr(dat, "index")) || !is.null(attr(dat[[1]], 
            "matrix")) || is.matrix(dat[[1]])) 
            ok <- FALSE
        if (ok) 
...
ziP <- function (theta = NULL, link = "identity", b = 0) 
{
    ...
    postproc <- function(family, y, prior.weights, fitted, linear.predictors, 
        offset, intercept) {
        posr <- list()
        posr$family <- paste("Zero inflated Poisson(", paste(round(family$getTheta(TRUE), 
            3), collapse = ","), ")", sep = "")
        ...
        meany <- mean(y)
        posr$null.deviance <- optimize(fnull, interval = c(meany/5, 
            meany * 3), family = family, y = y, wt = prior.weights)$objective - 
            sum(lf)
        posr
    }
    rd <- function(mu, wt, scale) {
...
nb <- function (theta = NULL, link = "log") 
{
    ...
    postproc <- function(family, y, prior.weights, fitted, linear.predictors, 
        ...
        posr$null.deviance <- find.null.dev(family, y, eta = linear.predictors, 
            offset, prior.weights)
        posr$family <- paste("Negative Binomial(", round(family$getTheta(TRUE), 
            3), ")", sep = "")
        posr
    }
    rd <- function(mu, wt, scale) {
...
gamlss.gH <- function (X, jj, l1, l2, i2, l3 = 0, i3 = 0, l4 = 0, i4 = 0, 
    d1b = 0, d2b = 0, deriv = 0, fh = NULL, D = NULL) 
{
    K <- length(jj)
    p <- ncol(X)
    n <- nrow(X)
    ...
    lb <- rep(0, p)
    for (i in 1:K) {
        lb[jj[[i]]] <- lb[jj[[i]]] + colSums(l1[, i] * X[, jj[[i]], 
            drop = FALSE])
    }
    lbb <- matrix(0, p, p)
    for (i in 1:K) for (j in i:K) {
        A <- crossprod(X[, jj[[i]], drop = FALSE], l2[, i2[i, 
            j]] * X[, jj[[j]], drop = FALSE])
        lbb[jj[[i]], jj[[j]]] <- lbb[jj[[i]], jj[[j]]] + A
        if (j > i) 
            lbb[jj[[j]], jj[[i]]] <- lbb[jj[[j]], jj[[i]]] + 
                t(A)
    }
    if (deriv > 0) {
        m <- ncol(d1b)
    ...
    if (deriv == 1) {
        ...
        for (l in 1:m) {
            for (i in 1:K) {
                ...
                d1H[jj[[i]], l] <- d1H[jj[[i]], l] + colSums(X[, 
                  jj[[i]], drop = FALSE] * (v * X[, jj[[i]], 
                  drop = FALSE]))
            }
        }
    }
    if (deriv > 1) {
        ...
        for (l in 1:m) {
            ...
            for (i in 1:K) for (j in i:K) {
                ...
                A <- crossprod(X[, jj[[i]], drop = FALSE], v * 
                  X[, jj[[j]], drop = FALSE])
                d1H[[l]][jj[[i]], jj[[j]]] <- d1H[[l]][jj[[i]], 
                  jj[[j]]] + A
                if (j > i) 
                  d1H[[l]][jj[[j]], jj[[i]]] <- d1H[[l]][jj[[j]], 
                    jj[[i]]] + t(A)
            }
        }
    }
    if (deriv > 2) {
        ...
        if (is.list(fh)) {
            ...
            d[d > 0] <- 1/d[d > 0]
            d[d <= 0] <- 0
            Xe <- t(D * ((fh$vectors %*% (d * t(fh$vectors))) %*% 
                (D * t(Xe))))
        }
        else {
            ipiv <- piv <- attr(fh, "pivot")
            ipiv[piv] <- 1:p
            Xe <- t(D * (backsolve(fh, forwardsolve(t(fh), (D * 
                t(Xe))[piv, ]))[ipiv, ]))
        }
    ...
    Sl = NULL, ...) 
{
    ...
    if (!scale.known && scoreType %in% c("REML", "ML", "EFS")) {
        nsp <- length(sp)
        scale <- exp(sp[nsp])
        sp <- sp[-nsp]
    }
    ...
    q <- ncol(x)
    if (length(UrS)) {
        grderiv <- if (scoreType == "EFS") 
            1
        else deriv * as.numeric(scoreType %in% c("REML", "ML", 
            "P-REML", "P-ML"))
    ...
    if (EMPTY) {
        ...
        for (iter in 1:control$maxit) {
            ...
            ng <- sum(good)
            zg[1:ng] <- z
            oo <- .C(C_pls_fit1, y = as.double(zg), X = as.double(x[good, 
        ...
        if (control$scale.est %in% c("pearson", "fletcher", "Pearson", 
            ...
            if (control$scale.est %in% c("fletcher", "Fletcher")) {
                s.bar = max(-0.9, mean(family$dvar(mu) * (y - 
                  mu) * sqrt(weights)/family$variance(mu)))
                if (is.finite(s.bar)) 
        ...
        if (scoreType %in% c("REML", "ML")) {
            ...
            REML <- (Dp/(2 * scale) - ls[1])/gamma + oo$rank.tol/2 - 
                rp$det/2 - remlInd * (Mp/2 * (log(2 * pi * scale) - 
                log(gamma)))
            attr(REML, "Dp") <- Dp/(2 * scale)
            if (deriv) {
                REML1 <- oo$D1/(2 * scale * gamma) + oo$trA1/2 - 
                  rp$det1/2
                if (deriv == 2) 
                  REML2 <- (matrix(oo$D2, nSp, nSp)/(scale * 
                    gamma) + matrix(oo$trA2, nSp, nSp) - rp$det2)/2
    ...
    list(coefficients = coef, residuals = residuals, fitted.values = mu, 
        ...
        ldetS1 = if (grderiv) rp$det1 else 0, scale.est = scale.est, 
        reml.scale = reml.scale, aic = aic.model, rank = oo$rank.est, 
        K = Kmat)
}
            ...
            if (type == "lpmatrix") {
                ...
                if (nlp > 1) {
                  if (is.null(fam$predict) || type == "link") {
                    ...
                    attr(X, "lpi") <- lpi
                    ffv <- fam$predict(fam, se.fit, y = response[start:stop], 
...
bam <- function (formula, family = gaussian(), data = list(), weights = NULL, 
    ...
    rho = 0, AR.start = NULL, discrete = FALSE, cluster = NULL, 
    nthreads = 1, gc.level = 1, use.chol = FALSE, samfrac = 1, 
    coef = NULL, drop.unused.levels = TRUE, G = NULL, fit = TRUE, 
    drop.intercept = NULL, ...) 
{
    ...
    if (is.null(G)) {
        ...
        if (is.null(family$family)) 
            stop("family not recognized")
        if (inherits(family, "general.family")) 
            stop("general families not supported by bam")
        if (family$family == "gaussian" && family$link == "identity") 
            am <- TRUE
        else am <- FALSE
        if (scale == 0) {
        ...
        if (discretize) {
            if (method != "fREML") {
                ...
                if (all(is.finite(nthreads)) && any(nthreads > 
                  1) && !mgcv.omp()) 
                  warning("openMP not available: single threaded computation only")
            }
        }
        if (inherits(family, "extended.family")) {
        ...
        mf <- match.call(expand.dots = FALSE)
        mf$formula <- gp$fake.formula
        mf$method <- mf$family <- mf$control <- mf$scale <- mf$knots <- mf$sp <- mf$min.sp <- mf$gc.level <- mf$gamma <- mf$paraPen <- mf$chunk.size <- mf$rho <- mf$cluster <- mf$discrete <- mf$use.chol <- mf$samfrac <- mf$nthreads <- mf$G <- mf$fit <- mf$select <- mf$drop.intercept <- mf$coef <- mf$... <- NULL
        mf$drop.unused.levels <- drop.unused.levels
        mf[[1]] <- quote(stats::model.frame)
        pmf <- mf
        pmf$formula <- gp$pf
        ...
        if (is.null(family$drop.intercept)) {
            if (is.null(drop.intercept)) 
                ...
                drop.intercept <- drop.intercept[1]
                if (drop.intercept) 
        ...
        while (reset) {
            G <- gam.setup(gp, pterms = pterms, data = mf0, knots = knots, 
                ...
                sparse.cons = sparse.cons, select = select, idLinksBases = !discretize, 
                scale.penalty = control$scalePenalty, paraPen = paraPen, 
                apply.by = !discretize, drop.intercept = drop.intercept, 
                modCon = 2)
        ...
        if (discretize) {
            ...
            if (drop.intercept) {
                xat <- attributes(G$Xd[[1]])
                ind <- xat$assign > 0
                G$Xd[[1]] <- G$Xd[[1]][, ind, drop = FALSE]
                xat$assign <- xat$assign[ind]
                xat$dimnames[[2]] <- xat$dimnames[[2]][ind]
                xat$dim[2] <- xat$dim[2] - 1
                attributes(G$Xd[[1]]) <- xat
            }
            if (ncol(G$Xd[[1]])) {
        ...
        if (is.null(G$offset)) 
            G$offset <- rep(0, n)
        if (!discretize && ncol(G$X) > nrow(mf)) 
    ...
    if (!is.finite(nthreads[1]) || nthreads[1] < 1) 
        nthreads[1] <- max(1, length(cluster))
    ...
    if (G$am && !G$discretize) {
        ...
        object <- bam.fit(G, mf, chunk.size, gp, scale, gamma, 
            ...
            use.chol = use.chol, npt = nthreads[1])
    }
    else if (G$discretize) {
        object <- bgam.fitd(G, mf, gp, scale, nobs.extra = 0, 
            rho = rho, coef = coef, control = control, npt = nthreads, 
            gc.level = gc.level, gamma = gamma, ...)
    }
    else {
        G$X <- matrix(0, 0, ncol(G$X))
        if (gc.level > 1) 
        ...
        if (samfrac < 1 && samfrac > 0) {
            ...
            if (length(ind) < 2 * ncol(G$X)) 
                ...
                object <- bgam.fit(G, mf[ind, ], chunk.size, 
                  ...
                  control = control1, cl = cluster, npt = nthreads[1], 
                  gc.level = gc.level, coef = coef, use.chol = use.chol, 
                  samfrac = 1, ...)
                G$w <- Gw
                G$offset <- Goffset
        ...
        object <- bgam.fit(G, mf, chunk.size, gp, scale, gamma, 
            ...
            cl = cluster, npt = nthreads[1], gc.level = gc.level, 
            use.chol = use.chol, ...)
    }
    if (gc.level > 0) 
        gc()
    ...
    object$residuals <- if (is.null(family$residuals)) 
        sqrt(family$dev.resids(object$y, object$fitted.values, 
            object$prior.weights)) * sign(object$y - object$fitted.values)
    else residuals(object)
    if (rho != 0) 
        object$std.rsd <- AR.resid(object$residuals, rho, object$model$"(AR.start)")
    if (!efam || is.null(object$deviance)) 
        object$deviance <- sum(object$residuals^2)
    dev <- object$deviance
    if (rho != 0 && family$family == "gaussian") 
        dev <- sum(object$std.rsd^2)
    object$aic <- if (efam) 
        family$aic(object$y, object$fitted.values, family$getTheta(), 
            object$prior.weights, dev)
    else family$aic(object$y, 1, object$fitted.values, object$prior.weights, 
        dev)
    object$aic <- object$aic - 2 * (length(object$y) - sum(sum(object$model[["(AR.start)"]]))) * 
        log(1/sqrt(1 - rho^2)) + 2 * sum(object$edf)
    if (!is.null(object$edf2) && sum(object$edf2) > sum(object$edf1)) 
        object$edf2 <- object$edf1
    if (is.null(object$null.deviance)) 
        object$null.deviance <- sum(family$dev.resids(object$y, 
            weighted.mean(object$y, object$prior.weights), object$prior.weights))
    if (!is.null(object$full.sp)) {
...
ginla <- function (G, A = NULL, nk = 16, nb = 100, J = 1, interactive = FALSE, 
    int = 0, approx = 0) 
{
    prog <- interactive() && interactive < 2
    if (!inherits(G, "gam.prefit") && !inherits(G, "bam.prefit")) 
        stop("Requires a gam or bam prefit object")
    if (int != 0) 
        G0 <- G
    if (inherits(G, "gam.prefit")) 
        b <- gam(G = G, method = "REML")
    else {
        if (!inherits(G, "bam.prefit")) 
            stop("Requires a gam or bam prefit object")
        if (is.null(G$Xd)) 
            stop("bam fits only supported with discrete==TRUE")
        b <- bam(G = G)
    }
    V <- sp.vcov(b, reg = 0.01)
    lsp <- log(b$sp)
    scale.estimated <- b$scale.estimated
    if (scale.estimated) 
        lsp <- c(lsp, log(b$sig2))
    if (inherits(b$family, "extended.family")) {
        n.theta <- if (is.null(b$family$n.theta)) 
            0
        else b$family$n.theta
        if (n.theta > 0) 
            lsp <- c(b$family$getTheta(), lsp)
    }
    else n.theta <- 0
    ...
    reml <- rep(0, nip)
    dens <- list()
    beta.lim <- matrix(0, ncol(X), 2)
    p <- ncol(b$Vp)
    if (!is.null(A)) {
        if (is.matrix(A) || length(A) == p) {
            B <- Acomp(A, is.null(G$Xd))
            pa <- nrow(A)
            kind <- 1:pa
        }
        else {
            A <- round(A)
            pa <- length(A)
            if (max(A) > p || min(A) < 1 || pa > p) 
                stop("something wrong with A index vector")
            kind <- A
            B <- list()
        }
    }
    ...
    iprog <- 0
    if (prog) 
        prg <- txtProgressBar(min = 0, max = (nip + 1) * pa, 
            initial = 0, char = "=", width = NA, title = "Progress", 
            style = 3)
    for (qq in 0:nip) {
        dens[[qq + 1]] <- list()
        if (qq > 0) {
            sp <- drop(t(rV) %*% ip$D[qq, ]) + lsp
            sp <- pmin(pmax(sp, lsp - 10), lsp + 10)
            wprior <- -sum((sp - lsp)^2/(200))
            if (n.theta > 0) {
                ii <- 1:n.theta
            ...
            else b <- bam(G = G0, sp = sp, scale = scale)
            G$sp <- sp
            reml[qq] <- -b$gcv.ubre + wprior
        }
        else max.reml <- -b$gcv.ubre
        G$family <- b$family
        G$sig2 <- b$sig2
        beta <- coef(b)
        if (!is.null(B$B)) {
        ...
        sd <- diag(b$Vp)^0.5
        BM <- matrix(0, p, nk)
        inla <- list(density = matrix(0, pa, nb), beta = matrix(0, 
            pa, nb))
        ldet <- dens0 <- rep(0, nk)
        eps <- 1e-04
        ...
        for (k in kind) {
            ...
            if (approx < 2) {
                ...
                R <- choldrop(R1, kd)
                pivk <- piv[-kd]
                pivk[pivk > k] <- pivk[pivk > k] - 1
                attr(R, "pivot") <- pivk
                attr(R, "dpc") <- dpc[-k]
                ldetH <- 2 * (sum(log(diag(R))) - sum(log(dpc[-k])))
            }
            bg <- qn * sd[k] + beta[k]
            BM[k, ] <- bg
            BM[-k, ] <- beta[-k] + b$Vp[-k, k] %*% ((t(bg) - 
                beta[k])/b$Vp[k, k])
            if (approx == 0) {
                db <- db0 <- beta * 0
                for (i in c((nk/2):1, (nk/2):nk)) {
                  ...
                  if (is.finite(nn$ll)) 
                    for (j in 1:20) {
                      if (max(abs(nn$dd[-k])) < 1e-04 * abs(nn$ll)) 
                        break
            ...
            else for (i in 1:nk) dens0[i] <- logf(BM[, i], G, 
                B$Bi, X, deriv = 0)$ll
            if (approx < 2) {
                if (J > 1) {
                  vb <- apply(BM, 1, var)
                ...
                step.length <- mean(colSums((BM - beta)^2)^0.5)/20
                D <- rep(c(-1, 1), J)
                for (i in 1:nk) if (is.finite(dens0[i])) {
                  ...
                  for (j in 1:J) {
                    h = H[-k, -k] %*% db[-k] + if (j > 1) 
                      u %*% (D[1:(2 * (j - 1))] * (t(u) %*% db[-k]))
                    else 0
                    g1 <- glogf(bm + db/2, G, B$Bi, X) - glogf(bm - 
                      db/2, G, B$Bi, X)
                    v <- cbind(h/sqrt(sum(db[-k] * h)), g1[-k]/sqrt(sum(db * 
                      g1)))
                    u <- if (j > 1) 
                      cbind(v, u)
                    else v
                  ...
                  ldet[i] <- (ldetH + as.numeric(determinant(diag(2 * 
                    J) + t(u) %*% Hu)$modulus))
                }
            }
            else ldet <- 0
            dens0 <- -dens0 - ldet/2
            dens0[!is.finite(dens0)] <- min(dens0, na.rm = TRUE) - 
                10
            dens0 <- dens0 - max(dens0)
            din <- interpSpline(bg, dens0)
            ok <- FALSE
            bg0 <- min(bg)
            bg1 <- max(bg)
            while (!ok) {
                inla$beta[kk, ] <- seq(bg0, bg1, length = nb)
                inla$density[kk, ] <- exp(predict(din, inla$beta[kk, 
                  ])$y)
                n.const <- sum(inla$density[kk, ]) * (inla$beta[kk, 
                  2] - inla$beta[kk, 1])
                inla$density[kk, ] <- inla$density[kk, ]/n.const
                maxd <- max(inla$density[kk, ])
                ok <- TRUE
                if (inla$density[kk, 1] > maxd * 0.005) {
                  ok <- FALSE
                  bg0 <- bg0 - sd[k]
                }
                if (inla$density[kk, nb] > maxd * 0.005) {
                  ok <- FALSE
                  bg1 <- bg1 + sd[k]
                }
            }
            din$coefficients[, 1] <- din$coefficients[, 1] - 
                log(n.const)
            dens[[qq + 1]][[kk]] <- din
            if (qq == 0 || bg0 < beta.lim[k, 1]) 
                beta.lim[k, 1] <- bg0
            if (qq == 0 || bg1 > beta.lim[k, 2]) 
                beta.lim[k, 2] <- bg1
            iprog <- iprog + 1
            if (prog) 
                setTxtProgressBar(prg, iprog)
            if (interactive) {
                plot(inla$beta[kk, ], inla$density[kk, ], ylim = range(inla$density[kk, 
                  ]) * 1.2, type = "l", col = 2, xlab = bquote(beta[.(k)]), 
                  ylab = "density")
                lines(inla$beta[kk, ], dnorm(inla$beta[kk, ], 
                  mean = beta[k], sd = sd[k]))
                if (interactive == 2) 
                  readline()
            }
    ...
    if (nip) {
        ...
        reml <- reml/sum(reml)
        for (k in 1:pa) {
            inla$beta[k, ] <- seq(beta.lim[k, 1], beta.lim[k, 
                2], length = nb)
            inla$density[k, ] <- exp(predict(dens[[1]][[k]], 
                inla$beta[k, ])$y) * reml[1]
        }
        for (qq in 2:(nip + 1)) {
            for (k in 1:pa) {
                inla$density[k, ] <- inla$density[k, ] + exp(predict(dens[[qq]][[k]], 
                  inla$beta[k, ])$y) * reml[qq]
            }
        }
        inla$reml <- reml
    }
...
cox.ph <- function (link = "identity") 
{
    ...
    preinitialize <- function(G) {
        G$family$data <- list()
        y.order <- if (is.matrix(G$y)) 
            order(G$y[, 2], G$y[, 1], decreasing = TRUE)
        else order(G$y, decreasing = TRUE)
        G$family$data$y.order <- y.order
        G$y <- if (is.matrix(G$y)) 
            G$y[y.order, ]
        else G$y[y.order]
    ...
    postproc <- expression({
        G$X <- Sl.initial.repara(G$Sl, G$X, inverse = TRUE, cov = FALSE, 
            both.sides = FALSE)
    ...
    hazard <- function(y, X, beta, wt, offset = 0) {
        if (is.matrix(y)) {
            ...
            ns <- length(strat.lev)
            nt <- 0
            for (i in 1:ns) nt <- nt + length(unique(y[strat == 
                strat.lev[i]]))
            tr.strat <- tr <- rep(0, nt)
            k <- 1
            for (i in 1:ns) {
                tr0 <- unique(y[strat == strat.lev[i]])
                ind <- k:(k + length(tr0) - 1)
                k <- k + length(tr0)
                tr[ind] <- tr0
                tr.strat[ind] <- strat.lev[i]
            }
        }
        else {
            ns <- 1
        ...
        if (ns == 1) {
            r <- match(y, tr)
            oo <- .C("coxpp", eta, A = as.double(X), as.integer(r), 
                d = as.integer(wt), h = as.double(rep(0, nt)), 
                q = as.double(rep(0, nt)), km = as.double(rep(0, 
                  nt)), n = as.integer(nrow(X)), p = as.integer(ncol(X)), 
                nt = as.integer(nt), PACKAGE = "mgcv")
            return(list(tr = tr, h = oo$h, q = oo$q, a = matrix(oo$A[1:(p * 
            ...
            for (i in 1:ns) {
                ...
                oo <- .C("coxpp", etai, A = as.double(X[ind, 
                  ]), as.integer(r0), d = as.integer(wt[ind]), 
                  h = as.double(rep(0, nti)), q = as.double(rep(0, 
                    nti)), km = as.double(rep(0, nti)), n = as.integer(length(ind)), 
                  p = as.integer(p), nt = as.integer(nti), PACKAGE = "mgcv")
                h[trind] <- oo$h
                q[trind] <- oo$q
                km[trind] <- oo$km
                r[ind] <- r0
            ...
            return(list(tr = tr, h = h, q = q, a = a, nt = nt, 
                r = r, km = km, strat = strat, tr.strat = tr.strat))
        }
    }
    ...
    predict <- function(family, se = FALSE, eta = NULL, y = NULL, 
        ...
        if (is.matrix(y)) {
            strat <- y[, 2]
            y <- y[, 1]
            if (is.null(family$data$strat)) 
                stop("something wrong with stratified prediction")
            ii <- order(strat, y, decreasing = TRUE)
            strat <- strat[ii]
        }
        else {
        ...
        if (is.null(off)) 
            off <- rep(0, n)
        if (is.null(strat)) {
            oo <- .C("coxpred", as.double(X), t = as.double(y), 
                as.double(beta), as.double(off), as.double(Vb), 
                a = as.double(family$data$a), h = as.double(family$data$h), 
                q = as.double(family$data$q), tr = as.double(family$data$tr), 
                n = as.integer(n), p = as.integer(ncol(X)), nt = as.integer(family$data$nt), 
                s = as.double(rep(0, n)), se = as.double(rep(0, 
                  n)), PACKAGE = "mgcv")
            s <- sef <- oo$s
            s[ii] <- oo$s
            sef[ii] <- oo$se
            ...
            for (i in 1:ns) {
                ind <- which(strat == pstrata[i])
                trind <- which(family$data$tr.strat == pstrata[i])
                n <- length(ind)
                oo <- .C("coxpred", as.double(X[ind, ]), t = as.double(y[ind]), 
                  as.double(beta), as.double(off), as.double(Vb), 
                  a = as.double(family$data$a[, trind]), h = as.double(family$data$h[trind]), 
                  q = as.double(family$data$q[trind]), tr = as.double(family$data$tr[trind]), 
    ...
    ll <- function(y, X, coef, wt, family, offset = NULL, deriv = 0, 
        ...
        for (j in 1:ns) {
            ind <- if (ns == 1) 
                1:length(y)
            else which(strat == strat.lev[j])
            tr <- unique(y[ind])
            r <- match(y[ind], tr)
            oo <- .C("coxlpl", as.double(mu[ind]), as.double(X[ind, 
                ]), as.integer(r), as.integer(wt[ind]), as.double(tr), 
                n = as.integer(length(y[ind])), p = as.integer(p), 
                nt = as.integer(length(tr)), lp = as.double(0), 
            ...
            if (deriv == 1) {
                ...
                for (i in 1:M) {
                  d1Ho[[i]] <- if (j == 1) 
                    matrix(oo$d1H[ind], p, p)
                  else matrix(oo$d1H[ind], p, p) + d1Ho[[i]]
                  ind <- ind + p^2
                }
            }
            if (deriv > 2) {
                d2Ho <- if (j == 1) 
                  matrix(oo$d2H, p, M * (M + 1)/2)
                else d2Ho + matrix(oo$d2H, p, M * (M + 1)/2)
            }
        }
        if (deriv > 2) {
            d <- ev$values
        ...
        list(l = lp, lb = lb, lbb = lbb, d1H = d1Ho, d2H = d2Ho, 
            trHid2H = trHid2H)
    }
    environment(ll) <- env
    ...
    scale, gamma, G, start = NULL, ...) 
{
    ...
    if (optimizer[2] %in% c("nlm.fd")) 
        .Deprecated(msg = paste("optimizer", optimizer[2], "is deprecated, please use newton or bfgs"))
    if (length(lsp) == 0) {
        optimizer[2] <- "no.sps"
    }
    ...
    if (optimizer[2] == "nlm.fd") {
        ...
        um <- nlm(full.score, lsp, typsize = lsp, fscale = fscale, 
            ...
            control = control, gamma = gamma, start = start, 
            ...)
        lsp <- um$estimate
    ...
    if (optimizer[1] == "efs" && optimizer[2] != "no.sps") {
        if (inherits(family, "general.family")) {
            object <- efsud(x = G$X, y = G$y, lsp = lsp, Sl = G$Sl, 
                weights = G$w, offset = G$offxset, family = family, 
                control = control, Mp = G$Mp, start = start)
        }
        else {
            family <- fix.family.ls(family)
            object <- efsudr(x = G$X, y = G$y, lsp = lsp, Eb = G$Eb, 
                UrS = G$UrS, weights = G$w, family = family, 
                offset = G$offset, start = start, U1 = G$U1, 
                intercept = TRUE, scale = scale, Mp = G$Mp, control = control, 
                n.true = G$n.true, ...)
        }
        ...
        if (optimizer[2] == "bfgs") 
            b <- bfgs(lsp = lsp, X = G$X, y = G$y, Eb = G$Eb, 
                ...
                start = start, pearson.extra = G$pearson.extra, 
                dev.extra = G$dev.extra, n.true = G$n.true, Sl = G$Sl, 
                ...)
            ...
            maxSstep = control$newton$maxSstep, maxHalf = control$newton$maxHalf, 
            printWarn = FALSE, scoreType = criterion, null.coef = G$null.coef, 
            start = start, pearson.extra = G$pearson.extra, dev.extra = G$dev.extra, 
            n.true = G$n.true, Sl = G$Sl, edge.correct = control$edge.correct, 
            ...)
        object <- b$object
        ...
        args <- list(X = G$X, y = G$y, Eb = G$Eb, UrS = G$UrS, 
            ...
            null.coef = G$null.coef, n.true = G$n.true, Sl = G$Sl, 
            start = start)
        if (optimizer[2] == "nlm") {
    ...
    object$method <- method
    if (inherits(family, "general.family")) {
        mv <- gam.fit5.post.proc(object, G$Sl, G$L, G$lsp0, G$S, 
            G$off)
    }
    else mv <- gam.fit3.post.proc(G$X, G$L, G$lsp0, G$S, G$off, 
        object)
    if (!is.null(mv$Vc)) 
        object$Vc <- mv$Vc
    if (!is.null(mv$edf2)) 
    ...
    if (inverse) {
        if (is.matrix(X)) {
            for (i in 1:nr) if (rp[[i]]$repara) {
                if (both.sides) 
                  X[rp[[i]]$ind, ] <- rp[[i]]$Qs %*% X[rp[[i]]$ind, 
                    , drop = FALSE]
                X[, rp[[i]]$ind] <- X[, rp[[i]]$ind, drop = FALSE] %*% 
                  t(rp[[i]]$Qs)
            }
        }
        else {
        ...
        if (is.matrix(X)) {
            for (i in 1:nr) if (rp[[i]]$repara) 
                X[, rp[[i]]$ind] <- X[, rp[[i]]$ind] %*% rp[[i]]$Qs
        }
        else {
            for (i in 1:nr) if (rp[[i]]$repara) 
                X[rp[[i]]$ind] <- t(rp[[i]]$Qs) %*% X[rp[[i]]$ind]
        }
    }
    X
}
    ...
    if (inverse) {
        if (is.matrix(X)) {
            if (cov) {
                for (b in 1:length(Sl)) if (Sl[[b]]$repara) {
                  ...
                  if (is.matrix(Sl[[b]]$D)) {
                    if (both.sides) 
                      X[ind, ] <- if (nt == 1) 
                        Sl[[b]]$D %*% X[ind, , drop = FALSE]
                      else pmmult(Sl[[b]]$D, X[ind, , drop = FALSE], 
                        FALSE, FALSE, nt = nt)
                    X[, ind] <- if (nt == 1) 
                      X[, ind, drop = FALSE] %*% t(Sl[[b]]$D)
                    else pmmult(X[, ind, drop = FALSE], Sl[[b]]$D, 
                      FALSE, TRUE, nt = nt)
                  }
                  else {
                ...
                for (b in 1:length(Sl)) if (Sl[[b]]$repara) {
                  ind <- Sl[[b]]$start:Sl[[b]]$stop
                  if (is.matrix(Sl[[b]]$D)) {
                    Di <- if (is.null(Sl[[b]]$Di)) 
                      t(Sl[[b]]$D)
                    else Sl[[b]]$Di
                    if (both.sides) 
                      X[ind, ] <- if (nt == 1) 
                        t(Di) %*% X[ind, , drop = FALSE]
                      else pmmult(Di, X[ind, , drop = FALSE], 
                        TRUE, FALSE, nt = nt)
                    X[, ind] <- if (nt == 1) 
                      X[, ind, drop = FALSE] %*% Di
                    else pmmult(X[, ind, drop = FALSE], Di, FALSE, 
                      FALSE, nt = nt)
                  }
                  else {
                    Di <- 1/Sl[[b]]$D
            ...
            for (b in 1:length(Sl)) if (Sl[[b]]$repara) {
                ...
                if (is.matrix(Sl[[b]]$D)) 
                  X[ind] <- Sl[[b]]$D %*% X[ind]
                else X[ind] <- Sl[[b]]$D * X[ind]
            }
        }
    }
    else for (b in 1:length(Sl)) if (Sl[[b]]$repara) {
        ind <- Sl[[b]]$start:Sl[[b]]$stop
        if (is.matrix(X)) {
            if (is.matrix(Sl[[b]]$D)) {
                if (both.sides) 
                  X[ind, ] <- if (nt == 1) 
                    t(Sl[[b]]$D) %*% X[ind, , drop = FALSE]
                  else pmmult(Sl[[b]]$D, X[ind, , drop = FALSE], 
                    TRUE, FALSE, nt = nt)
                X[, ind] <- if (nt == 1) 
                  X[, ind, drop = FALSE] %*% Sl[[b]]$D
                else pmmult(X[, ind, drop = FALSE], Sl[[b]]$D, 
                  FALSE, FALSE, nt = nt)
            }
            else {
            ...
            if (both.sides) {
                if (is.matrix(Sl[[b]]$D)) 
                  X[ind] <- t(Sl[[b]]$D) %*% X[ind]
                else X[ind] <- Sl[[b]]$D * X[ind]
            }
            else {
                if (is.matrix(Sl[[b]]$D)) 
                  X[ind] <- if (is.null(Sl[[b]]$Di)) 
                    t(Sl[[b]]$D) %*% X[ind]
                  else Sl[[b]]$Di %*% X[ind]
                else X[ind] <- X[ind]/Sl[[b]]$D
            }
        }
    }
    X
...
twlss <- function (link = list("log", "identity", "identity"), a = 1.01, 
    b = 1.99) 
{
    if (length(link) != 3) 
        stop("gevlss requires 3 links specified as character strings")
    okLinks <- list(c("log", "identity", "sqrt"), "identity", 
        c("identity"))
    stats <- list()
    for (i in 1:3) {
        if (link[[i]] %in% okLinks[[i]]) 
            stats[[i]] <- make.link(link[[i]])
        else stop(link[[i]], " link not available for mu parameter of twlss")
        fam <- structure(list(link = link[[i]], canonical = "none", 
            linkfun = stats[[i]]$linkfun, mu.eta = stats[[i]]$mu.eta), 
            class = "family")
        fam <- fix.family.link(fam)
    ...
    residuals <- function(object, type = c("deviance", "pearson", 
        ...
        if (type == "pearson") {
            ...
            rsd <- sign(object$y - mu) * sqrt(pmax(2 * (object$y * 
                theta - kappa) * object$prior.weights/phi, 0))
        }
        return(rsd)
    }
    postproc <- expression({
        tw.para <- tw.null.fit(object$y)
        tw.y1 <- object$y + (object$y == 0)
        tw.theta <- (tw.y1^(1 - tw.para[2]) - tw.para[1]^(1 - 
            tw.para[2]))/(1 - tw.para[2])
        tw.kappa <- (object$y^(2 - tw.para[2]) - tw.para[1]^(2 - 
            tw.para[2]))/(2 - tw.para[2])
        object$null.deviance <- sum(pmax(2 * (object$y * tw.theta - 
            tw.kappa) * object$prior.weights/exp(object$fitted.values[, 
            3]), 0))
    })
    ll <- function(y, X, coef, wt, family, offset = NULL, deriv = 0, 
        d1b = 0, d2b = 0, Hp = NULL, rank = 0, fh = NULL, D = NULL) {
        ...
        for (i in 1:3) if (is.null(offset[[i]])) 
            offset[[i]] <- 0
        jj <- attr(X, "lpi")
        eta <- X[, jj[[1]], drop = FALSE] %*% coef[jj[[1]]] + 
            offset[[1]]
        mu <- family$linfo[[1]]$linkinv(eta)
        theta <- X[, jj[[2]], drop = FALSE] %*% coef[jj[[2]]] + 
            offset[[2]]
        rho <- X[, jj[[3]], drop = FALSE] %*% coef[jj[[3]]] + 
            offset[[3]]
        a <- get(".a")
        b <- get(".b")
        ld <- ldTweedie(y, mu = mu, p = NA, phi = NA, rho = rho, 
            theta = theta, a = a, b = b, all.derivs = TRUE)
        l <- sum(ld[, 1])
        l1 <- cbind(ld[, 7], ld[, 4], ld[, 2])
        l2 <- cbind(ld[, 8], ld[, 9], ld[, 10], ld[, 5], ld[, 
            6], ld[, 3])
    ...
    initialize <- expression({
        ...
        if (is.null(start)) {
            ...
            start[jj[[1]]] <- startji
            mu1 <- family$linfo[[1]]$linkinv(x[, jj[[1]], drop = FALSE] %*% 
                start[jj[[1]]])
    ...
    structure(list(family = "twlss", ll = ll, link = paste(link), 
        nlp = 3, tri = trind.generator(3), initialize = initialize, 
        postproc = postproc, residuals = residuals, linfo = stats, 
        d2link = 1, d3link = 1, d4link = 1, ls = 1, available.derivs = 0), 
        class = c("general.family", "extended.family", "family"))
}
...
negbin <- function (theta = stop("'theta' must be specified"), link = "log") 
{
    ...
    structure(list(family = famname, link = linktemp, linkfun = stats$linkfun, 
        ...
        getTheta = getTheta, qf = qf, rd = rd, canonical = ""), 
        class = "family")
}
    ...
    drop.unused.levels = TRUE, drop.intercept = NULL, ...) 
{
    ...
    if (is.null(G)) {
        ...
        mf <- match.call(expand.dots = FALSE)
        mf$formula <- gp$fake.formula
        mf$family <- mf$control <- mf$scale <- mf$knots <- mf$sp <- mf$min.sp <- mf$H <- mf$select <- mf$drop.intercept <- mf$gamma <- mf$method <- mf$fit <- mf$paraPen <- mf$G <- mf$optimizer <- mf$in.out <- mf$... <- NULL
        mf$drop.unused.levels <- drop.unused.levels
        mf[[1]] <- quote(stats::model.frame)
        pmf <- mf
        mf <- eval(mf, parent.frame())
...
gam.reparam <- function (rS, lsp, deriv) 
{
    ...
    oo <- .C(C_get_stableS, S = as.double(matrix(0, q, q)), Qs = as.double(matrix(0, 
        q, q)), sp = as.double(exp(lsp)), rS = as.double(unlist(rS)), 
        rSncol = as.integer(rSncol), q = as.integer(q), M = as.integer(M), 
        deriv = as.integer(deriv), det = as.double(0), det1 = as.double(rep(0, 
            M)), det2 = as.double(matrix(0, M, M)), d.tol = as.double(d.tol), 
        r.tol = as.double(r.tol), fixed_penalty = as.integer(fixed.penalty))
    S <- matrix(oo$S, q, q)
    S <- (S + t(S)) * 0.5
    p <- abs(diag(S))^0.5
    p[p == 0] <- 1
...
bam.update <- function (b, data, chunk.size = 10000) 
{
    ...
    X <- predict(b, newdata = mf, type = "lpmatrix", na.action = b$NA.action)
    rownames(X) <- NULL
    ...
    if (method == "GCV.Cp") {
        ...
        object <- list(coefficients = res$beta, edf = res$edf, 
            ...
            rank = ncol(um$X), Ve = res$Ve, Vp = res$Vp, Vc = res$Vc, 
...
smooth.construct.tensor.smooth.spec <- function (object, data, knots) 
{
    ...
    if (inter) {
        ...
        object$sparse.cons <- if (is.null(object$sparse.cons)) 
            rep(0, m)
        else object$sparse.cons
    }
    else {
    ...
    mono <- rep(FALSE, m)
    for (i in 1:m) {
        if (!is.null(object$margin[[i]]$mono) && object$margin[[i]]$mono != 
            0) 
            mono[i] <- TRUE
        knt <- dat <- list()
        term <- object$margin[[i]]$term
        ...
        object$margin[[i]] <- if (object$mc[i]) 
            smoothCon(object$margin[[i]], dat, knt, absorb.cons = TRUE, 
                n = length(dat[[1]]), sparse.cons = object$sparse.cons[i])[[1]]
    ...
    if (object$np) 
        for (i in 1:m) {
            if (object$margin[[i]]$dim == 1) {
                if (is.null(object$margin[[i]]$noterp)) {
    ...
    if (!is.null(object$margin[[1]]$xt$dropu) && object$margin[[1]]$xt$dropu) {
        ...
        S <- tensor.prod.penalties(Sm)
        for (i in 1:m) {
            D <- S[[i]][rowSums(S[[i]][, -ind, drop = FALSE]) == 
                0, ind]
            r[i] <- nrow(D)
            S[[i]] <- crossprod(D)
        }
...
sp.vcov <- function (x, edge.correct = TRUE, reg = 0.001) 
{
    if (!inherits(x, "gam")) 
...
cholup <- function (R, u, up = TRUE) 
{
    ...
    if (up == 0) 
        if ((n > 1 && R1[2, 1] < -1) || (n == 1 && u[1] > R[1])) 
            stop("update not positive definite")
    R1
}
...
smooth.construct.bs.smooth.spec <- function (object, data, knots) 
{
    ...
    if (!is.null(k) && length(k) == 4 && length(k) < nk + 2 * 
        m[1]) {
        k <- sort(k)
        dx <- (k[4] - k[1])/(nk - 1)
        ko <- c(k[1] - dx * m[1], k[4] + dx * m[1])
        k <- c(seq(ko[1], k[1], length = m[1] + 1), seq(k[2], 
            k[3], length = max(0, nk - 2)), seq(k[4], ko[2], 
            length = m[1] + 1))
    }
    ...
    if (length(unique(m2)) < length(m2)) 
        stop("multiple penalties of the same order is silly")
    for (i in 1:length(m2)) {
        ...
        if (pord == 0) {
            ...
            P <- solve(matrix(rep(seq(-1, 1, length = pord + 
                1), pord + 1)^rep(0:pord, each = pord + 1), pord + 
                1, pord + 1))
            i1 <- rep(1:(pord + 1), pord + 1) + rep(1:(pord + 
                1), each = pord + 1)
            H <- matrix((1 + (-1)^(i1 - 2))/(i1 - 1), pord + 
                1, pord + 1)
            W1 <- t(P) %*% H %*% P
            h <- h/2
            ld0 <- rep(sdiag(W1), length(h)) * rep(h, each = pord + 
                1)
            i1 <- c(rep(1:pord, length(h)) + rep(0:(length(h) - 
                1) * (pord + 1), each = pord), length(ld0))
            ld <- ld0[i1]
    ...
    object$rank <- object$bs.dim - m2
    object$null.space.dim <- min(m2)
    object
}
    ...
    deriv = 2) 
{
            ...
            if (length(Sl[[b]]$S) == 1) {
                ...
                if (root) {
                  if (Sl[[b]]$repara) {
                    ...
                    diag(E)[ind] <- exp(rho[k.sp] * 0.5)
                  }
                  else {
                    D <- Sl[[b]]$Di[1:Sl[[b]]$rank, ]
                    indc <- Sl[[b]]$start:(Sl[[b]]$start + ncol(D) - 
                      1)
                    indr <- Sl[[b]]$start:(Sl[[b]]$start + nrow(D) - 
                      1)
                    E[indr, indc] <- D * exp(rho[k.sp] * 0.5)
                  }
                }
                ...
                ind <- k.sp:(k.sp + m - 1)
                grp <- if (repara) 
                  gam.reparam(Sl[[b]]$rS, lsp = rho[ind], deriv = deriv)
                else ldetSblock(Sl[[b]]$rS, rho[ind], deriv = deriv, 
                  root = root, nt = nt)
                Sl[[b]]$lambda <- exp(rho[ind])
                ldS <- ldS + grp$det
                grp$det1 <- grp$det1[!fixed[ind]]
                grp$det2 <- if (deriv > 1) 
                  grp$det2[!fixed[ind], !fixed[ind]]
                else 0
                nd <- length(grp$det1)
                if (nd > 0) {
                ...
                if (repara) {
                  rp[[k.rp]] <- list(block = b, ind = (Sl[[b]]$start:Sl[[b]]$stop)[Sl[[b]]$ind], 
                    Qs = grp$Qs, repara = Sl[[b]]$repara)
                  k.rp <- k.rp + 1
                  for (i in 1:m) {
                    Sl[[b]]$Srp[[i]] <- Sl[[b]]$lambda[i] * grp$rS[[i]] %*% 
                      t(grp$rS[[i]])
                  }
                }
                k.sp <- k.sp + m
                if (Sl[[b]]$repara) {
                  if (root) {
                    ic <- Sl[[b]]$start:(Sl[[b]]$start + ncol(grp$E) - 
                      1)
                    ir <- Sl[[b]]$start:(Sl[[b]]$start + nrow(grp$E) - 
                      1)
                    E[ir, ic] <- grp$E
                  ...
                  for (i in 2:m) {
                    Sl[[b]]$St <- Sl[[b]]$St + Sl[[b]]$lambda[i] * 
                      Sl[[b]]$S[[i]]
                  }
                  if (root) {
                    Eb <- t(mroot(Sl[[b]]$St, Sl[[b]]$rank))
                    indc <- Sl[[b]]$start:(Sl[[b]]$start + ncol(Eb) - 
                      1)
                    indr <- Sl[[b]]$start:(Sl[[b]]$start + nrow(Eb) - 
                      1)
                    E[indr, indc] <- Eb
                  }
                }
    ...
    list(ldetS = ldS, ldet1 = d1.ldS, ldet2 = d2.ldS, Sl = Sl, 
        rp = rp, E = E)
}
...
gaulss <- function (link = list("identity", "logb"), b = 0.01) 
{
    ...
    if (link[[2]] %in% okLinks[[2]]) {
        ...
        stats[[2]]$mu.eta <- eval(parse(text = paste("function(eta) { ee <- exp(eta); -ee/(ee +", 
            b, ")^2 }")))
        stats[[2]]$d2link <- eval(parse(text = paste("function(mu) { mub <- pmax(1 - mu *", 
            b, ",.Machine$double.eps);(2*mub-1)/(mub*mu)^2}")))
        stats[[2]]$d3link <- eval(parse(text = paste("function(mu) { mub <-  pmax(1 - mu *", 
            b, ",.Machine$double.eps);((1-mub)*mub*6-2)/(mub*mu)^3}")))
        stats[[2]]$d4link <- eval(parse(text = paste("function(mu) { mub <- pmax(1 - mu *", 
            b, ",.Machine$double.eps);(((24*mub-36)*mub+24)*mub-6)/(mub*mu)^4}")))
    }
    ...
    structure(list(family = "gaulss", ll = ll, link = paste(link), 
        ...
        postproc = postproc, residuals = residuals, linfo = stats, 
        rd = rd, d2link = 1, d3link = 1, d4link = 1, ls = 1, 
        available.derivs = 2), class = c("general.family", "extended.family", 
        "family"))
}
...
gam.fit5.post.proc <- function (object, Sl, L, lsp0, S, off) 
{
    ...
    if (attr(R, "rank") < ncol(R)) {
        ...
        while (retry) {
            ...
            lbb <- crossprod(R)
            Hp <- lbb + object$D * t(object$D * object$St)
        ...
        ipiv[piv] <- 1:p
        R <- t(t(R[, ipiv])/object$D)
    }
    ipiv <- piv <- attr(object$L, "pivot")
    ipiv[piv] <- 1:p
    Vb <- crossprod(forwardsolve(t(object$L), diag(object$D, 
        nrow = p)[piv, , drop = FALSE])[ipiv, , drop = FALSE])
    if (sum(object$bdrop)) {
    ...
    if (!is.null(object$outer.info$hess) && !is.null(object$db.drho)) {
        ...
        edge.correct <- if (is.null(attr(hess, "edge.correct"))) 
            FALSE
        else TRUE
        K <- if (edge.correct) 
            2
        ...
        for (k in 1:K) {
            ...
            d[ind] <- 0
            d[!ind] <- 1/sqrt(d[!ind])
            Vc <- crossprod((d * t(ev$vectors)) %*% t(db.drho))
            d <- ev$values
            d[ind] <- 0
            d <- if (k == 1) 
                1/sqrt(d + 1/50)
        ...
        rp <- if (edge.correct) 
            attr(object$outer.info$hess, "rp")
        else object$rp
    ...
    edf <- diag(F)
    if (!is.null(object$outer.info$hess) && !is.null(object$db.drho)) {
        Vc <- Vc + Vb.corr(R, L, lsp0, S, off, dw = NULL, w = NULL, 
            lsp, Vr)
        if (edge.correct) 
            Vc1 <- Vc1 + Vb.corr(R, L, lsp0, S, off, dw = NULL, 
                w = NULL, lsp1, Vr1)
        else Vc1 <- Vc
    }
    edf1 <- 2 * edf - rowSums(t(F) * F)
    edf2 <- if (edge.correct) 
        rowSums(Vc1 * crossprod(R))
    else rowSums(Vc * crossprod(R))
    if (sum(edf2) > sum(edf1)) 
        edf2 <- edf1
    list(Vc = Vc, Vb = Vb, Ve = Ve, edf = edf, edf1 = edf1, edf2 = edf2, 
        F = F, R = R)
}
...
Sl.setup <- function (G) 
{
    ...
    b <- 1
    if (G$n.paraPen) {
        off <- unique(G$off[1:G$n.paraPen])
        for (i in 1:length(off)) {
            ind <- (1:G$n.paraPen)[G$off[1:G$n.paraPen] %in% 
                off[i]]
            if (length(ind) > 1) {
                nr <- 0
                for (k in 1:length(ind)) nr <- max(nr, nrow(G$S[[ind[k]]]))
                Sl[[b]] <- list()
                Sl[[b]]$S <- list()
                St <- matrix(0, nr, nr)
                ...
                Sl[[b]]$lambda <- rep(1, length(ind))
                Sl[[b]]$repara <- FALSE
            }
            else {
                Sl[[b]] <- list(start = off[ind], stop = off[ind] + 
                  nrow(G$S[[ind]]) - 1, rank = G$rank[ind], S = list(G$S[[ind]]))
                Sl[[b]]$S <- list(G$S[[ind]])
                Sl[[b]]$lambda <- 1
                Sl[[b]]$repara <- TRUE
            }
    ...
    if (length(G$smooth)) 
        for (i in 1:length(G$smooth)) {
            if (!is.null(G$smooth[[i]]$fixed) && G$smooth[[i]]$fixed) 
                m <- 0
            else m <- length(G$smooth[[i]]$S)
            if (m > 0) {
                Sl[[b]] <- list()
                Sl[[b]]$start <- G$smooth[[i]]$first.para
                Sl[[b]]$stop <- G$smooth[[i]]$last.para
                Sl[[b]]$repara <- if (is.null(G$smooth[[i]]$g.index)) 
                  TRUE
                else FALSE
            }
            if (m == 0) {
                ...
                for (j in 1:m) {
                  sbdiag[j] <- sum(abs(Sl[[b]]$S[[j]][ut])) == 
                    0
                  ir <- range((1:nb)[rowSums(abs(Sl[[b]]$S[[j]])) > 
                    0])
                  sbStart[j] <- ir[1]
                  sbStop[j] <- ir[2]
                }
                ...
                for (j in 1:m) {
                  ...
                  if (all(sbdiag)) {
                    for (k in 1:m) if (j != k) 
                      itot[diag(Sl[[b]]$S[[k]]) != 0] <- TRUE
                    if (sum(itot[diag(Sl[[b]]$S[[j]]) != 0]) > 
                      0) {
                      split.ok <- FALSE
                      break
                    }
                    ...
                    for (k in 1:m) if (j != k) 
                      itot[sbStart[k]:sbStop[k]] <- TRUE
                    if (sum(itot[sbStart[j]:sbStop[j]]) > 0) {
                      split.ok <- FALSE
                      break
                    }
                  }
                ...
                if (split.ok) {
                  for (j in 1:m) {
                    ...
                    Sl[[b]]$S <- list(G$smooth[[i]]$S[[j]][ind, 
                      ind, drop = FALSE])
                    Sl[[b]]$start <- G$smooth[[i]]$first.para + 
                      sbStart[j] - 1
                    Sl[[b]]$stop <- G$smooth[[i]]$first.para + 
                      sbStop[j] - 1
                    Sl[[b]]$rank <- G$smooth[[i]]$rank[j]
                    Sl[[b]]$lambda <- 1
                    Sl[[b]]$repara <- TRUE
                    if (!is.null(G$smooth[[i]]$g.index)) {
                      if (any(G$smooth[[i]]$g.index[ind])) 
                        Sl[[b]]$repara <- FALSE
                    }
                    b <- b + 1
                  }
    ...
    for (b in 1:length(Sl)) {
        if (length(Sl[[b]]$S) == 1) {
            if (sum(abs(Sl[[b]]$S[[1]][upper.tri(Sl[[b]]$S[[1]], 
                diag = FALSE)])) == 0) {
                D <- diag(Sl[[b]]$S[[1]])
                ind <- D > 0
                D[ind] <- 1/sqrt(D[ind])
                ...
                U <- es$vectors
                D <- es$values
                if (is.null(Sl[[b]]$rank)) {
                  Sl[[b]]$rank <- sum(D > .Machine$double.eps^0.8 * 
                    max(D))
                }
                ind <- rep(FALSE, length(D))
                ind[1:Sl[[b]]$rank] <- TRUE
                D[ind] <- 1/sqrt(D[ind])
                D[!ind] <- 1
                Sl[[b]]$D <- t(D * t(U))
                Sl[[b]]$Di <- t(U)/D
            ...
            Sl[[b]]$rS <- list()
            S <- Sl[[b]]$S[[1]]
            for (j in 2:length(Sl[[b]]$S)) S <- S + Sl[[b]]$S[[j]]
            ...
            indc <- Sl[[b]]$start:(Sl[[b]]$start + ncol(St) - 
                1)
            indr <- Sl[[b]]$start:(Sl[[b]]$start + nrow(St) - 
                1)
            E[indr, indc] <- St
        }
    }
    attr(Sl, "E") <- E
    attr(Sl, "lambda") <- lambda
    Sl
}
...
{
    if (!is.factor(data[[object$term]])) 
        warning("argument of mrf should be a factor variable")
    ...
    if (is.null(object$xt$penalty)) {
        ...
        else if (!is.numeric(object$xt$nb[[1]])) {
...
gam.control <- function (nthreads = 1, irls.reg = 0, epsilon = 1e-07, maxit = 200, 
    ...
    efs.tol = 0.1, keepData = FALSE, scale.est = "fletcher", 
    edge.correct = FALSE) 
{
    scale.est <- match.arg(scale.est, c("fletcher", "pearson", 
        "deviance"))
    if (!is.logical(edge.correct) && (!is.numeric(edge.correct) || 
        edge.correct < 0)) 
        stop("edge.correct must be logical or a positive number")
    ...
    list(nthreads = round(nthreads), irls.reg = irls.reg, epsilon = epsilon, 
        ...
        scalePenalty = scalePenalty, efs.lspmax = efs.lspmax, 
        efs.tol = efs.tol, keepData = as.logical(keepData[1]), 
        scale.est = scale.est, edge.correct = edge.correct)
}
...
ziplss <- function (link = list("identity", "identity")) 
{
    ...
    predict <- function(family, se = FALSE, eta = NULL, y = NULL, 
        ...
        if (is.null(eta)) {
            if (is.null(off)) 
                off <- list(0, 0)
            off[[3]] <- 0
            for (i in 1:2) if (is.null(off[[i]])) 
                off[[i]] <- 0
            lpi <- attr(X, "lpi")
            X1 <- X[, lpi[[1]], drop = FALSE]
            X2 <- X[, lpi[[2]], drop = FALSE]
            gamma <- drop(X1 %*% beta[lpi[[1]]] + off[[1]])
            eta <- drop(X2 %*% beta[lpi[[2]]] + off[[2]])
            if (se) {
    ...
    ll <- function(y, X, coef, wt, family, offset = NULL, deriv = 0, 
        ...
        for (i in 1:2) if (is.null(offset[[i]])) 
            offset[[i]] <- 0
        jj <- attr(X, "lpi")
        eta <- X[, jj[[1]], drop = FALSE] %*% coef[jj[[1]]] + 
            offset[[1]]
...
interpret.gam <- function (gf, extra.special = NULL) 
{
    if (is.list(gf)) {
        ...
        if (length(av) > 0) {
            ...
            ret$fake.formula[[2]] <- fff[[2]]
        }
        else ret$fake.formula <- ret[[1]]$fake.formula
...
smooth.construct.sos.smooth.spec <- function (object, data, knots) 
{
    ...
    if (nk == 0) {
        xu <- uniquecombs(matrix(x, n, 2), TRUE)
        nu <- nrow(xu)
        if (n > xtra$max.knots) {
            if (nu > xtra$max.knots) {
                seed <- try(get(".Random.seed", envir = .GlobalEnv), 
...
uniquecombs <- function (x, ordered = FALSE) 
{
    ...
    if (inherits(x, "data.frame")) {
        ...
        for (i in 1:length(x)) {
            ...
            if (is.factor(xo[[i]]) || is.logical(xo[[i]])) 
                x[[i]] <- as.numeric(xo[[i]])
            if (!is.numeric(x[[i]])) 
                recheck <- TRUE
        }
    ...
    if (ncol(x) == 1) {
        ...
        txt <- paste("paste0(", paste("x[,", 1:ncol(x), "]", 
            sep = "", collapse = ",\"*\","), ")", sep = "")
    ...
    if (recheck) {
        if (all.equal(xoo, x[ind, ], check.attributes = FALSE) != 
            TRUE) 
...
pcls <- function (M) 
{
    ...
    o <- .C(C_RPCLS, as.double(M$X), as.double(M$p), as.double(M$y), 
        ...
        as.double(Sa), as.integer(M$off), as.integer(df), as.double(M$sp), 
    ...
    ...) 
{
    ...
    if (m > 0) {
        ...
        for (i in 1:m) {
            ...
            edf1i <- edfi <- sum(object$edf[start:stop])
            if (!is.null(object$edf1)) 
                edf1i <- sum(object$edf1[start:stop])
            ...
            if (!fx && object$smooth[[i]]$null.space.dim == 0 && 
                !is.null(object$R)) {
                res <- if (re.test) 
                  reTest(object, i)
                else NULL
                ...
                res <- testStat(p, Xt, V, min(ncol(Xt), edf1i), 
                  type = 0, res.df = rdf)
            }
            if (!is.null(res)) {
                ii <- ii + 1
...
ldTweedie <- function (y, mu = y, p = 1.5, phi = 1, rho = NA, theta = NA, 
    a = 1.001, b = 1.999, all.derivs = FALSE) 
{
    n <- length(y)
    if (all(!is.na(rho)) && all(!is.na(theta))) {
        if (a >= b || a <= 1 || b >= 2) 
            stop("1<a<b<2 (strict) required")
        work.param <- TRUE
        buffer <- if (length(unique(theta)) == 1 && length(unique(rho)) == 
            1) 
            TRUE
        else FALSE
        ...
        dpth2[ind] <- ((a - b) * ethi + (b - a) * ethi^2)/(ethi + 
            1)^3
        dpth2[!ind] <- ((a - b) * ethni^2 + (b - a) * ethni)/(ethni + 
            1)^3
    }
    else {
        work.param <- FALSE
        if (all.derivs) 
            warning("all.derivs only available in rho, theta parameterization")
        buffer <- if (length(unique(p)) == 1 && length(unique(phi)) == 
            1) 
            TRUE
        else FALSE
        rho <- log(phi)
        if (min(p) >= 1 && max(p) <= 2) {
            ...
            if (sum(ind)) {
                ...
                dthp2[ind] <- (pabp[ind] + 1)/((p.ind - a) * 
                  (b - p.ind)) - (pabp[ind] + 1)/(p.ind - a)^2
            }
        }
    }
    if (min(p) < 1 || max(p) > 2) 
    ...
    if (sum(ind)) {
        ...
        ld[ind, 1] <- dgamma(y[ind], shape = 1/phi[ind], rate = 1/(phi[ind] * 
            mu[ind]), log = TRUE)
        ld[ind, 2] <- (digamma(1/phi[ind]) + log(phi[ind]) - 
            1 + y[ind]/mu[ind] - log(y[ind]/mu[ind]))/(phi[ind] * 
            phi[ind])
        ld[ind, 3] <- -2 * ld[ind, 2]/phi[ind] + (1 - trigamma(1/phi[ind])/phi[ind])/(phi[ind]^3)
    }
    ind <- p == 1
    if (sum(ind)) {
        if (all.equal(y[ind]/phi[ind], round(y[ind]/phi[ind])) != 
            TRUE) 
            stop("y must be an integer multiple of phi for Tweedie(p=1)")
        indi <- (y[ind] != 0) | (mu[ind] != 0)
        bkt <- y[ind] * 0
        bkt[indi] <- ((y[ind])[indi] * log((mu[ind]/phi[ind])[indi]) - 
            (mu[ind])[indi])
        dig <- digamma(y[ind]/phi[ind] + 1)
        trig <- trigamma(y[ind]/phi[ind] + 1)
        ld[ind, 1] <- bkt/phi[ind] - lgamma(y[ind]/phi[ind] + 
            1)
        ld[ind, 2] <- (-bkt - y[ind] + dig[ind] * y[ind])/(phi[ind]^2)
        ld[ind, 3] <- (2 * bkt + 3 * y[ind] - 2 * dig * y[ind] - 
            trig * y[ind]^2/phi[ind])/(phi[ind]^3)
    }
    ind <- y == 0 & p > 1 & p < 2
    ld[ind, ] <- 0
    ind <- ind & mu > 0
    if (sum(ind)) {
        ...
        ld[ind, 2] <- -ld[ind, 1]/phii
        ld[ind, 3] <- -2 * ld[ind, 2]/phii
        ld[ind, 4] <- -ld[ind, 1] * (log(mu.ind) - 1/(2 - p.ind))
        ld[ind, 5] <- 2 * ld[ind, 4]/(2 - p.ind) + ld[ind, 1] * 
            log(mu.ind)^2
        ld[ind, 6] <- -ld[ind, 4]/phii
        if (work.param && all.derivs) {
            mup <- mu.ind^p.ind
    ...
    if (length(ind) > 0) {
        if (buffer) {
            oo <- .C(C_tweedious, w = as.double(w), w1 = as.double(w1), 
                w2 = as.double(w2), w1p = as.double(y * 0), w2p = as.double(y * 
                  0), w2pp = as.double(y * 0), y = as.double(y), 
                eps = as.double(.Machine$double.eps^2), n = as.integer(length(y)), 
                th = as.double(theta[1]), rho = as.double(rho[1]), 
            ...
            if (length(rho) != n) 
                rho <- array(rho, dim = n)
            oo <- .C(C_tweedious2, w = as.double(w), w1 = as.double(w1), 
                w2 = as.double(w2), w1p = as.double(y * 0), w2p = as.double(y * 
                  0), w2pp = as.double(y * 0), y = as.double(y), 
                eps = as.double(.Machine$double.eps^2), n = as.integer(length(y)), 
                th = as.double(theta[ind]), rho = as.double(rho[ind]), 
                a = as.double(a), b = as.double(b))
        }
        if (oo$eps < -0.5) {
            if (oo$eps < -1.5) {
                oo$w2 <- oo$w1 <- oo$w2p <- oo$w1p <- oo$w2pp <- rep(NA, 
                  length(y))
            }
        ...
        if (!work.param) {
            if (length(dthp1) != n) 
                dthp1 <- array(dthp1, dim = n)
            if (length(dthp2) != n) 
                dthp2 <- array(dthp2, dim = n)
            dthp1i <- dthp1[ind]
        ...
        twop <- 2 - p
        mu1p <- theta <- mu^onep
        k.theta <- mu * theta/twop
        theta <- theta/onep
        a1 <- (y/onep - mu/twop)
        l.base <- mu1p * a1/phii
        ld[ind, 1] <- l.base - log(y)
        ld[ind, 2] <- -l.base/phii
        ld[ind, 3] <- 2 * l.base/(phii^2)
        x <- theta * y * (1/onep - log.mu)/phii + k.theta * (log.mu - 
            1/twop)/phii
        ld[ind, 4] <- x
        ld[ind, 5] <- theta * y * (log.mu^2 - 2 * log.mu/onep + 
            2/onep^2)/phii - k.theta * (log.mu^2 - 2 * log.mu/twop + 
            2/twop^2)/phii
        ld[ind, 6] <- -x/phii
    }
    if (work.param) {
        ...
        ld[, 4] <- ld[, 4] * dpth1
        ld[, 6] <- ld[, 6] * dpth1 * phi
        colnames(ld)[1:6] <- c("l", "rho", "rho.2", "th", "th.2", 
            "th.rho")
    }
    if (work.param && all.derivs && length(ind) > 0) {
        a2 <- mu1p/(mu * phii)
        ld[ind, 7] <- a2 * (onep * a1 - mu/twop)
        ld[ind, 8] <- -a2 * (onep * p * a1/mu + 2 * onep/twop)
        ld[ind, 9] <- a2 * (-log.mu * onep * a1 - a1 + onep * 
            (y/onep^2 - mu/twop^2) + mu * log.mu/twop - mu/twop^2)
        ld[ind, 10] <- a2 * (mu/(phii * twop) - onep * a1/phii)
        ld[, 10] <- ld[, 10] * phi
        ld[, 9] <- ld[, 9] * dpth1
        colnames(ld) <- c("l", "rho", "rho.2", "th", "th.2", 
            "th.rho", "mu", "mu.2", "mu.theta", "mu.rho")
    }
    if (length(ind) > 0) {
...
fix.family.ls <- function (fam) 
{
    ...
    else if (family == "quasi" || family == "quasipoisson" || 
...
.skeleton_package_title = "Mixed GAM Computation Vehicle with Automatic SmoothnessEstimation"

.skeleton_package_version = "1.8-28"

.skeleton_package_depends = "nlme"

.skeleton_package_imports = "methods,stats,graphics,Matrix,splines,utils"
