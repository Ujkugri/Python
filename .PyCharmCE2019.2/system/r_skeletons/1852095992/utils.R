...
setWinProgressBar <- function (pb, value, title = NULL, label = NULL) 
{
    if (!inherits(pb, "winProgressBar")) 
        stop(gettextf("'pb' is not from class %s", dQuote("winProgressBar")), 
            domain = NA)
    if (!is.null(title)) 
...
RShowDoc <- function (what, type = c("pdf", "html", "txt"), package) 
{
    ...
    if (what == "NEWS") {
        ...
        docs <- docs[vapply(docs, function(x) file_test("-f", 
            x), NA)]
...
recover <- function () 
{
    ...
    if (from > 0L) {
        ...
        else if (isFALSE(getOption("show.error.messages"))) 
            return(NULL)
...
osVersion <- "Windows 10 x64 (build 17134)"
    ...
    info = character()) 
{
    ...
    if (method == "none") 
        ...
        uri <- paste0("mailto:", address, "?subject=", subject, 
...
getClipboardFormats <- function (numeric = FALSE) 
{
    known <- c("text", "bitmap", "metafile PICT", "SYLK", "DIF", 
        "TIFF", "OEM text", "DIB", "palette", "pendata", "RIFF", 
        "audio", "Unicode text", "enhanced metafile", "drag-and-drop", 
        "locale", "shell")
...
.romans <- c(M = 1000L, CM = 900L, D = 500L, CD = 400L, C = 100L, XC = 90L, 
L = 50L, XL = 40L, X = 10L, IX = 9L, V = 5L, IV = 4L, I = 1L)
    ...
    ...) 
{
    ...
    if (method == "auto") {
        ...
        method <- if (startsWith(url, "ftps:") && capabilities("libcurl")) 
            "libcurl"
        else "wininet"
    }
    nh <- names(headers)
    ...
    switch(method, internal = , wininet = {
        if (!is.null(headers)) headers <- paste0(nh, ": ", headers, 
            "\r\n", collapse = "")
...
SweaveSyntaxLatex <- structure(list(doc = "^[[:space:]]*\\\\end\\{Scode\\}", code = "^[[:space:]]*\\\\begin\\{Scode\\}\\{?([^}]*)\\}?.*", 
    ...
    input = "^[[:space:]]*\\\\SweaveInput\\{([^}]*)\\}", trans = list(
        ...
        input = "\\\\SweaveInput{\\1}")), class = "SweaveSyntax")
...
Filters <- structure(c("R or S files (*.R,*.q,*.ssc,*.S)", "Enhanced metafiles (*.emf)", 
"Postscript files (*.ps)", "PDF files (*.pdf)", "Png files (*.png)", 
"Windows bitmap files (*.bmp)", "Jpeg files (*.jpeg,*.jpg)", 
"Text files (*.txt)", "R images (*.RData,*.rda)", "Zip files (*.zip)", 
"Package tarballs (*.tar.gz)", "All files (*.*)", "*.R;*.q;*.ssc;*.S", 
"*.emf", "*.ps", "*.pdf", "*.png", "*.bmp", "*.jpeg;*.jpg", "*.txt", 
"*.RData;*.rda", "*.zip", "*.tar.gz", "*.*"), .Dim = c(12L, 2L
), .Dimnames = list(c("R", "emf", "ps", "pdf", "png", "bmp", 
"jpeg", "txt", "RData", "zip", "tarball", "All"), NULL))
...
arrangeWindows <- function (action = c("vertical", "horizontal", "cascade", "minimize", 
    "restore"), windows, preserve = TRUE, outer = FALSE) 
{
    action <- match.arg(action)
    action <- which(action == c("cascade", "horizontal", "vertical", 
        "minimize", "restore"))
    stopifnot(length(action) == 1 && !is.na(action))
    if (missing(windows)) {
        args <- if (!is.null(a <- get0(".arrangeWindowsDefaults", 
            globalenv()))) 
            a
        else list()
        if (action == 5) 
    ...
    role = NULL, comment = NULL, first = NULL, last = NULL) 
{
    ...
    person1 <- function(given = NULL, family = NULL, middle = NULL, 
        ...
        if (length(comment)) {
            ind <- grepl(paste0("^https?://orcid.org/", "([[:digit:]]{4}[-]){3}[[:digit:]]{3}[[:alnum:]]$"), 
                comment)
            if (any(ind)) {
...
getWindowsHandle <- function (which = "Console") 
{
    if (is.numeric(which)) {
        ...
        if (which > 0 && which <= length(.Devices) && .Devices[[which]] != 
            "windows") 
            return(NULL)
    }
...
winProgressBar <- function (title = "R progress bar", label = "", min = 0, max = 1, 
    initial = 0, width = 300L) 
{
    res <- .External2(C_winProgressBar, as.integer(width), as.character(title), 
        as.character(label), as.double(min), as.double(max), 
        as.double(initial))
...
readRegistry <- function (key, hive = c("HLM", "HCR", "HCU", "HU", "HCC", "HPD"), 
    maxdepth = 1, view = c("default", "32-bit", "64-bit")) 
{
    view <- match(match.arg(view), c("default", "32-bit", "64-bit"))
    .External2(C_readRegistry, key, match.arg(hive), maxdepth, 
        view)
}
...
package.skeleton <- function (name = "anRpackage", list = character(), environment = .GlobalEnv, 
    path = ".", force = FALSE, code_files = character(), encoding = "unknown") 
{
...
loadRconsole <- function (file = choose.files(file.path(Sys.getenv("R_USER"), 
    "Rconsole"))) 
    ...
    ...) 
{
    ...
    MyWrap <- function(...) paste(paste(strwrap(paste(...)), 
        collapse = "\n"))
    ...
    if ("otherPkgs" %in% names(inf)) {
        ...
        other <- askYesNo(MyWrap("You have packages", paste0("(", 
    ...
    title <- try(grep("<title>", readLines(page, 10L), fixed = TRUE, 
        value = TRUE), silent = TRUE)
